...
$launcherFileName = "start_exact_approved_slow_liquidity_listing_announcement_discovery_visible.ps1"
$runtimeModule = Join-Path $repoRoot "trading_mvp\src\slow_liquidity_listing_announcement_discovery.py"
...
$planPath = Join-Path $repoRoot "docs\plans\slow-liquidity-listing-announcement-discovery-planonly-20260816.json"
...
$runId = "slow_liquidity_listing_announcement_discovery_20260816"
$launchRecordPath = Join-Path $repoRoot "docs\agent-log\run-gates\$runId.launch.json"
$outputPath = "E:\ZolotyayLopata-data\exports\trading-mvp\slow-liquidity-listing-announcement-discovery\$runId"
$expectedPlanHash = "1a7e4505e611b505e23c98cd89be015dc04d14b2da7cf3df12085a21db9ec8db"
$expectedPlanFileSha256 = "4ed3124faf5abb26db95963aad316cccd98006945ae0bd5f1bd32ef07325bfa8"
$maxRuntimeSec = 180
...
function Assert-FrozenPlan {
    ...
    if (
        ...
        $raw -notmatch "www\.mexc\.com/announcements" -or
        $raw -notmatch "www\.gate\.com/announcements"
    ) {
...
function Invoke-FullPreflight {
    ...
    if (Test-Path -LiteralPath (Join-Path $outputPath "listing-announcement-candidates.json") -PathType Leaf) {
...
function New-GlobalWriterClaim {
    ...
    $raw = @(& $python $writerClaimCli `
        ...
        "--owner-kind" "slow_liquidity_listing_announcement_discovery" `
...
function Assert-CompletedOutput {
    $recordsPath = Join-Path $outputPath "listing-announcement-candidates.json"
    $manifestPath = Join-Path $outputPath "manifest.json"
    if (-not (Test-Path -LiteralPath $recordsPath -PathType Leaf)) {
        throw "listing-announcement-candidates.json is missing."
    }
    ...
    if ([string]$manifest.status -ne "LISTING_ANNOUNCEMENT_DISCOVERY_INCOMPLETE") {
