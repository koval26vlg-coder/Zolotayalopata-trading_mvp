...
$launcherFileName = "start_exact_approved_slow_liquidity_spot_v2_official_currency_json_visible.ps1"
$runtimeModule = Join-Path $repoRoot "trading_mvp\src\slow_liquidity_spot_v2_official_currency_json.py"
...
$planPath = Join-Path $repoRoot "docs\plans\slow-liquidity-spot-v2-official-currency-json-planonly-20260815.json"
...
$runId = "slow_liquidity_spot_v2_official_currency_json_20260815"
$launchRecordPath = Join-Path $repoRoot "docs\agent-log\run-gates\$runId.launch.json"
$outputPath = "E:\ZolotyayLopata-data\exports\trading-mvp\slow-liquidity-spot-v2-official-currency-json\$runId"
$expectedPlanHash = "b6db2d430d42728681594701e00ddeb95f302f5728e99f198daccadc930fc9fc"
$expectedPlanFileSha256 = "7e0820f23dd34cf8a70084193e97505650191bc2e9ed9ee3e0a4d713282d5f48"
$maxRuntimeSec = 300
...
function Test-VisibleConsoleWindow {
    ...
    if ($null -eq ("TradingMvp.SpotV2CurrencyJsonVisibleConsoleNative" -as [type])) {
        Add-Type -TypeDefinition @'
using System;
using System.Runtime.InteropServices;

namespace TradingMvp {
    public static class SpotV2CurrencyJsonVisibleConsoleNative {
...
'@
    ...
    $consoleWindow = [TradingMvp.SpotV2CurrencyJsonVisibleConsoleNative]::GetConsoleWindow()
    return (
        $consoleWindow -ne [IntPtr]::Zero -and
        [TradingMvp.SpotV2CurrencyJsonVisibleConsoleNative]::IsWindowVisible($consoleWindow)
    )
}
...
function Assert-FrozenPlan {
    if ((Get-Sha256 -Path $planPath) -cne $expectedPlanFileSha256) {
        throw "Frozen currency-json plan file SHA256 mismatch."
    }
    ...
    if (
        ...
        [string]$plan.evidence_class -cne "OFFICIAL_PUBLIC_REST_CURRENCY_JSON" -or
        [bool]$plan.identity_evidence -or
        [bool]$plan.identity_verdict_allowed -or
        [string]$plan.mode -cne "PlanOnly"
    ) {
        throw "Frozen currency-json plan identity mismatch."
    }
    ...
    if (
        ...
        $raw -match "sitemap\.xml" -or
        $raw -match "/sitemaps/" -or
        $raw -notmatch "OFFICIAL_PUBLIC_REST_CURRENCY_JSON" -or
        $raw -notmatch "api\.gateio\.ws/api/v4/spot/currencies/"
    ) {
        throw "Frozen currency-json plan leaked page locator, v7, or perp bindings."
    }
    return $plan
}
...
function Invoke-FullPreflight {
    ...
    if (Test-Path -LiteralPath (Join-Path $outputPath "gate-currency-records.json") -PathType Leaf) {
        throw "Immutable currency-json output already exists."
    }
...
function New-GlobalWriterClaim {
    ...
    $raw = @(& $python $writerClaimCli `
        ...
        "--owner-kind" "slow_liquidity_spot_v2_official_currency_json" `
...
function Assert-CompletedOutput {
    ...
    if (-not (Test-Path -LiteralPath $recordsPath -PathType Leaf)) {
        throw "Currency-json gate-currency-records.json is missing."
    }
    ...
    if ([string]$manifest.status -ne "SPOT_V2_OFFICIAL_CURRENCY_JSON_INCOMPLETE") {
...
try {
    ...
    Write-Host "[spot-v2-currency-json] visible exact worker claimed: $runId" -ForegroundColor Cyan
    ...
    Write-Host "[spot-v2-currency-json] running; pid=$($runtimeProcess.Id); cap=${maxRuntimeSec}s" -ForegroundColor Green
    ...
    while (-not $runtimeProcess.HasExited) {
        ...
        Write-Host ("[spot-v2-currency-json] elapsed={0:n1}s writer_pid={1}" -f $stopwatch.Elapsed.TotalSeconds, $runtimeProcess.Id)
    ...
    Write-Host "[spot-v2-currency-json] $($completed.status); identity_verdict=false; retry=false" -ForegroundColor Green
} catch {
    ...
    Write-Host "[spot-v2-currency-json] STOPPED_INCOMPLETE; retry is not authorized." -ForegroundColor Red
    throw
} finally {
    if ($globalClaimToken) {
        try {
            ...
            Write-Host "[spot-v2-currency-json] writer claim release failed: $($_.Exception.Message)" -ForegroundColor Red
        }
    }
}
