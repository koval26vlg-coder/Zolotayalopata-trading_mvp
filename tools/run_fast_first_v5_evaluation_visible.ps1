...
param(
    [string]$PlanPath = "",
    [string]$ExpectedPlanHash = "d553c0120b0c3fcb3e4ff80d097bb8270788f690ab703c4b9c3c92d75db5277c",
    [string]$OutputPath = "",
...
$DataRoot = "E:\ZolotyayLopata-data\exports\trading-mvp\fast-edge-v5"
...
if (-not $PlanPath) {
    $PlanPath = Join-Path $DataRoot "plans\fast_first_wick_rejection_reversal_planonly_20260714_140353.json"
}
if (-not $RunId) {
    $RunId = "fast_first_v5_wick_rejection_reversal_$(Get-Date -Format 'yyyyMMdd_HHmmss')"
}
if (-not $OutputPath) {
...
function Get-NextDecision {
    ...
    if ($Status -eq "RUNNING") { return "FAST_FIRST_V5_EVALUATION_RUNNING" }
    if ($Status -eq "STOPPED_INCOMPLETE") { return "FAST_FIRST_V5_EVALUATION_STOPPED_INCOMPLETE" }
    switch ($Verdict) {
        "ACCEPT_FOR_SHORT_EXECUTION_PROBE" { return "FAST_FIRST_V5_ACCEPT_FOR_SHORT_EXECUTION_PROBE" }
        "REJECT" { return "FAST_FIRST_V5_REJECTED" }
        default { return "FAST_FIRST_V5_INSUFFICIENT_DATA" }
    }
}
...
function Update-RunGate {
    ...
    foreach ($gatePath in $GatePaths) {
        ...
        $fields = [ordered]@{
            ...
            output = [ordered]@{ path = $OutputPath; type = "fast_first_v5_wick_rejection_reversal_evaluation" }
            ...
            next_goal_reason = "Frozen venue-local wick-rejection reversal hypothesis evaluated without grid search or OOS tuning."
...
if (-not $Worker) {
    ...
    $launchPlan = [ordered]@{
        schema = "fast_first_v5_visible_evaluation_launch_v1"
    ...
    Write-JsonAtomic -Path $LaunchRecordPath -Value $launchPlan
    Write-Host "Visible Fast-First v5 evaluation opened. PID=$($process.Id)" -ForegroundColor Green
...
try { $host.UI.RawUI.WindowTitle = "trading_mvp Fast-First v5 OOS - $RunId" } catch { }
Write-Host "trading_mvp Fast-First v5: visible sealed no-grid OOS evaluation" -ForegroundColor Yellow
...
try {
    ...
    $manifest = [ordered]@{
        schema = "fast_first_v5_wick_rejection_reversal_manifest_v1"
        run_id = $RunId
        project = "trading_mvp"
        final = $true
        completed_cycles = 2
    ...
    Write-JsonAtomic -Path $failurePath -Value ([ordered]@{
        schema = "fast_first_v5_evaluation_failure_v1"
        run_id = $RunId
        final = $false
        created_at = (Get-Date).ToString("o")
        error = $message
