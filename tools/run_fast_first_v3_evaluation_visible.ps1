...
param(
    [string]$PlanPath = "",
    [string]$ExpectedPlanHash = "3f086ac9c0f59c9690a63870f03ba44543559e08271333e73ae7957e86e240f7",
    [string]$OutputPath = "",
...
$DataRoot = "E:\ZolotyayLopata-data\exports\trading-mvp\fast-edge-v3"
...
if (-not $PlanPath) {
    $PlanPath = Join-Path $DataRoot "plans\fast_first_lottery_max_planonly_20260713.json"
}
if (-not $RunId) {
    $RunId = "fast_first_v3_lottery_max_$(Get-Date -Format 'yyyyMMdd_HHmmss')"
}
if (-not $OutputPath) {
...
function Get-NextDecision {
    ...
    if ($Status -eq "RUNNING") { return "FAST_FIRST_V3_EVALUATION_RUNNING" }
    if ($Status -eq "STOPPED_INCOMPLETE") { return "FAST_FIRST_V3_EVALUATION_STOPPED_INCOMPLETE" }
    switch ($Verdict) {
        "ACCEPT_FOR_SHORT_EXECUTION_PROBE" { return "FAST_FIRST_V3_ACCEPT_FOR_SHORT_EXECUTION_PROBE" }
        "REJECT" { return "FAST_FIRST_V3_REJECTED" }
        default { return "FAST_FIRST_V3_INSUFFICIENT_DATA" }
    }
}
...
function Update-RunGate {
    ...
    foreach ($gatePath in $GatePaths) {
        ...
        $fields = [ordered]@{
            ...
            output = [ordered]@{ path = $OutputPath; type = "fast_first_v3_lottery_max_evaluation" }
            ...
            next_goal_reason = "Frozen lottery-MAX hypothesis evaluated without grid search or OOS tuning."
...
if (-not $Worker) {
    ...
    $launchPlan = [ordered]@{
        schema = "fast_first_v3_visible_evaluation_launch_v1"
    ...
    Write-Host "Visible Fast-First v3 evaluation opened. PID=$($process.Id)" -ForegroundColor Green
...
try { $host.UI.RawUI.WindowTitle = "trading_mvp Fast-First v3 OOS - $RunId" } catch { }
Write-Host "trading_mvp Fast-First v3: visible sealed no-grid OOS evaluation" -ForegroundColor Yellow
...
try {
    ...
    $manifest = [ordered]@{
        schema = "fast_first_v3_lottery_max_manifest_v1"
        run_id = $RunId
        project = "trading_mvp"
        final = $true
        completed_cycles = 2
    ...
    Write-JsonAtomic -Path $failurePath -Value ([ordered]@{
        schema = "fast_first_v3_evaluation_failure_v1"
        run_id = $RunId
        final = $false
        created_at = (Get-Date).ToString("o")
        error = $message
