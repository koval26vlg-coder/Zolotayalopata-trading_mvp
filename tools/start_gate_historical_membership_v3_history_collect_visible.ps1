...
$PlanModule = Join-Path $ProjectRoot "trading_mvp\src\gate_historical_membership_v3_history_plan.py"
$CollectorModule = Join-Path $ProjectRoot "trading_mvp\src\gate_historical_membership_v3_history_collector.py"
...
function Set-RunState {
    ...
    foreach ($entry in @(
        ...
        @("next_step_after_ready", $(if ($Final) { "Create hash-bound membership-v3 history quality PlanOnly; do not evaluate returns yet." } else { "Status-only until the visible history collect finishes." }))
...
$WriterLockPath = Join-Path $OutputRoot ".membership-v3-history-writer.lock"
...
spec = importlib.util.spec_from_file_location("gate_membership_v3_history_plan_launch", module_file)
...
if ($LASTEXITCODE -ne 0) { throw "Membership-v3 history PlanOnly validation failed." }
...
if ($Worker) {
    ...
    try { $Host.UI.RawUI.WindowTitle = "trading_mvp Gate membership-v3 history - $RunId" } catch { }
    ...
    Set-RunState -Status "RUNNING" -Final $false -WorkerPid $PID -TotalTasks @($plan.archive_tasks).Count -NextDecision "GATE_MEMBERSHIP_V3_HISTORY_COLLECT_RUNNING"
    Write-Host "trading_mvp visible Gate membership-v3 history archive collect" -ForegroundColor Cyan
    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0) { throw "membership-v3-history collector exited with code $LASTEXITCODE" }
        ...
        try { Set-RunState -Status "STOPPED_INCOMPLETE" -Final $false -CompletedTasks $completed -TotalTasks $total -Errors 1 -StopReason "gate_membership_v3_history_collect_failed" -Failure $message -NextDecision "VISIBLE_RESUME_SAME_HISTORY_RUN_ID_REQUIRED" } catch { }
        Remove-OwnedWriterLock
...
if ($PlanOnly) {
    [ordered]@{
        schema = "trading_mvp_gate_membership_v3_history_visible_collect_preview_v1"
...
$record = [ordered]@{
    schema = "trading_mvp_gate_membership_v3_history_visible_launch_v1"; project = "trading_mvp"; run_id = $RunId
...
try {
    ...
    Set-RunState -Status "RUNNING" -Final $false -WorkerPid $process.Id -TotalTasks @($plan.archive_tasks).Count -NextDecision "GATE_MEMBERSHIP_V3_HISTORY_COLLECT_RUNNING"
