...
param(
    [string]$PlanPath = "",
    [string]$ExpectedPlanHash = "5396885aa9abf77a461f20aa190c843b86be098b76abd6f3a5655a8f725eee60",
    [string]$OutputPath = "",
...
$DataRoot = "E:\ZolotyayLopata-data\exports\trading-mvp\fast-edge-v4"
...
if (-not $PlanPath) {
    $PlanPath = Join-Path $DataRoot "plans\fast_first_funding_pressure_reversal_planonly_night_policy_20260714_121647.json"
}
if (-not $RunId) {
    $RunId = "fast_first_v4_funding_pressure_reversal_$(Get-Date -Format 'yyyyMMdd_HHmmss')"
}
if (-not $OutputPath) {
...
function Get-NextDecision {
    ...
    if ($Status -eq "RUNNING") { return "FAST_FIRST_V4_EVALUATION_RUNNING" }
    if ($Status -eq "STOPPED_INCOMPLETE") { return "FAST_FIRST_V4_EVALUATION_STOPPED_INCOMPLETE" }
    switch ($Verdict) {
        "ACCEPT_FOR_SHORT_EXECUTION_PROBE" { return "FAST_FIRST_V4_ACCEPT_FOR_SHORT_EXECUTION_PROBE" }
        "REJECT" { return "FAST_FIRST_V4_REJECTED" }
        default { return "FAST_FIRST_V4_INSUFFICIENT_DATA" }
    }
}
...
function Update-RunGate {
    ...
    foreach ($gatePath in $GatePaths) {
        ...
        $fields = [ordered]@{
            ...
            output = [ordered]@{ path = $OutputPath; type = "fast_first_v4_funding_pressure_reversal_evaluation" }
            ...
            next_goal_reason = "Frozen venue-local funding-pressure reversal hypothesis evaluated without grid search or OOS tuning."
...
function Get-ApprovedDeadline {
    if (-not $ApprovedNotLaterThan) {
        throw "ApprovedNotLaterThan is required for an actual visible evaluation."
    }
    try {
...
if (-not $PlanOnly -and -not $ConfirmedResearchRun) {
    throw "Actual OOS evaluation requires -ConfirmedResearchRun, an explicit duration and ApprovedNotLaterThan."
}
if ($Worker) {
...
if (-not $Worker) {
    ...
    $launchPlan = [ordered]@{
        schema = "fast_first_v4_visible_evaluation_launch_v1"
    ...
    Write-JsonAtomic -Path $LaunchRecordPath -Value $launchPlan
    Write-Host "Visible Fast-First v4 evaluation opened. PID=$($process.Id)" -ForegroundColor Green
...
try { $host.UI.RawUI.WindowTitle = "trading_mvp Fast-First v4 OOS - $RunId" } catch { }
Write-Host "trading_mvp Fast-First v4: visible sealed no-grid OOS evaluation" -ForegroundColor Yellow
...
try {
    ...
    $manifest = [ordered]@{
        schema = "fast_first_v4_funding_pressure_reversal_manifest_v1"
        run_id = $RunId
        project = "trading_mvp"
        final = $true
        completed_cycles = 2
    ...
    Write-JsonAtomic -Path $failurePath -Value ([ordered]@{
        schema = "fast_first_v4_evaluation_failure_v1"
        run_id = $RunId
        final = $false
        created_at = (Get-Date).ToString("o")
        error = $message
