...
$runtimeModule = "trading_mvp.src.slow_liquidity_identity_request_plan_discovery_v4"
...
$runId = "slow_liquidity_identity_request_plan_discovery_20260816_v4"
...
if (-not $ExecutionManifestPath) {
    $ExecutionManifestPath = Join-Path $repoRoot `
        "docs\plans\slow-liquidity-identity-request-plan-discovery-execution-manifest-20260816-v4.json"
}
if (-not $OutputPath) {
    $OutputPath = `
...
function Invoke-FullPreflight {
    ...
    if ([string]$guard.status -ne "ACTIVE" -or [string]$guard.decision -ne "RUN_SLOW_LIQUIDITY_IDENTITY_REQUEST_PLAN_DISCOVERY_V4" -or [string]$guard.usage.status -ne "AVAILABLE" -or [string]$guard.usage.decision -ne "CONTINUE" -or [double]$guard.usage.remaining_percent -le 15) {
...
if ($Status) {
    [ordered]@{
        schema = "trading_mvp_slow_liquidity_identity_request_plan_discovery_status_v4"
        run_id = $runId
...
if (-not $VisibleWorker) {
    ...
    [ordered]@{
    schema = "trading_mvp_slow_liquidity_identity_request_plan_discovery_visible_launch_v4"
...
if ([string]$preflight.status -ne "READY_FOR_VISIBLE_SINGLE_USE_REQUEST_PLAN_DISCOVERY_RUN") {
    ...
    schema = "trading_mvp_slow_liquidity_identity_request_plan_discovery_launch_v4"
...
try {
    ...
    $capability = [ordered]@{
        schema = "trading_mvp_slow_liquidity_identity_request_plan_discovery_launcher_capability_v4"
        status = "ACTIVE"
        run_id = $runId
        owner_pid = $PID
