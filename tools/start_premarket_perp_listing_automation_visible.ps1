...
$defaultPlanPath = Join-Path $repoRoot "docs\plans\premarket-perp-listing-impulse-planonly-20260824-v4.json"
...
$statePath = Join-Path $runGateDir "premarket_perp_listing_automation_state.json"
$ledgerPath = Join-Path $runGateDir "premarket_perp_listing_automation_attempts.jsonl"
...
$launchRecordPath = Join-Path $runGateDir "premarket_perp_listing_automation.launch.json"
$workerErrorPath = Join-Path $runGateDir "premarket_perp_listing_automation.worker-error.log"
$pythonHandoffDir = Join-Path $runGateDir "python-worker-handoffs"
$eventsPath = Join-Path $repoRoot "exports\trading-mvp\premarket-perp\raw_events.jsonl"
...
function Enter-VisibleLaunchMutex([int]$WaitMilliseconds = 0) {
    $mutexName = "Local\ZolotyayLopataPremarketLaunch_" + (Get-TextSha256 ([System.IO.Path]::GetFullPath($statePath))).Substring(0, 24)
...
function Get-NextIntervalUtc($State = $null) {
    return [DateTimeOffset]::UtcNow.AddSeconds((Get-IntervalSeconds $State)).ToString("o")
}
...
function Write-JsonFile([string]$Path, $Payload) {
    ...
    $Payload | ConvertTo-Json -Depth 30 | Set-Content -LiteralPath $tmpPath -Encoding UTF8
...
function Append-Ledger($Payload) {
    ...
    if ($dir -and -not (Test-Path -LiteralPath $dir)) { New-Item -ItemType Directory -Force -Path $dir | Out-Null }
    ($Payload | ConvertTo-Json -Compress -Depth 30) | Add-Content -LiteralPath $ledgerPath -Encoding UTF8
}

function Get-DefaultState {
    return [ordered]@{
        schema = "trading_mvp_premarket_perp_listing_automation_state_v1"
        ...
        outcomes = [ordered]@{}
        accrual = [ordered]@{ contracts_seen = 0; events_written = 0; complete_events = 0 }
        last_error = $null
...
function Get-State {
    ...
    if ([string]$state.schema -ne "trading_mvp_premarket_perp_listing_automation_state_v1") { throw "automation state schema mismatch" }
...
function Acquire-Claim([string]$RunId, [string]$PlanHash) {
    ...
    try {
        ...
        $claimArgs = @(
            ...
            "--owner-kind", "premarket_perp_visible_worker",
            "--plan-hash", $PlanHash,
...
function Invoke-PlanPreflight {
    ...
    $planCheckText = & $pythonExe $planValidator --plan $PlanPath --json 2>&1 | Out-String
    $planExitCode = $LASTEXITCODE
    if ($planExitCode -ne 0) { throw "Plan validator exited $planExitCode`: $($planCheckText.Trim())" }
    if ([string]::IsNullOrWhiteSpace($planCheckText)) { throw "Plan validator returned empty output" }
    $planCheck = $planCheckText.Trim() | ConvertFrom-Json -ErrorAction Stop
    ...
    if (([bool]$planCheck.ok -and [string]$planCheck.status -cne "PLAN_OK") -or
        (-not [bool]$planCheck.ok -and [string]$planCheck.status -cne "PLAN_INVALID")) {
        throw "Plan validator payload status is inconsistent"
    }
    if ([string]$planCheck.plan_hash -cnotmatch '^[0-9a-f]{64}$') { throw "Plan validator payload plan_hash is invalid" }
    $gateCheck = Invoke-ActiveRunGateCheck
    $gate = $gateCheck.payload
    $reasons = [System.Collections.Generic.List[string]]::new()
    if (-not [bool]$planCheck.ok) { $reasons.Add("plan_check_failed:" + (@($planCheck.reasons) -join ",")) }
    ...
    return [ordered]@{
        ...
        reasons = @($reasons)
        plan_id = [string]$planCheck.plan_id
        plan_hash = [string]$planCheck.plan_hash
...
function Get-StatusPayload {
    $state = Get-State
    $launch = $null
    if (Test-Path -LiteralPath $launchRecordPath) { try { $launch = Read-JsonFile -Path $launchRecordPath } catch { $launch = @{ status = "UNREADABLE" } } }
    return [ordered]@{
...
if ($Status) {
    ...
    if ($Json) { $payload | ConvertTo-Json -Depth 30 } else { Write-Host ($payload | ConvertTo-Json -Depth 30) }
    exit 0
}
...
if ($PreflightOnly) {
    ...
    if ($Json) { $preflight | ConvertTo-Json -Depth 30 } else { Write-Host ($preflight | ConvertTo-Json -Depth 30) }
...
if ($VisibleWorker) {
    ...
    $launchRecord = [ordered]@{
        schema = "trading_mvp_premarket_perp_listing_automation_launch_v1"
        status = "RUNNING"
        automation_id = $automationId
        attempt_id = $attemptId
        visible_terminal_pid = $PID
        worker_pid = $PID
        runner_mode = "visible_terminal_worker"
        started_at_utc = $started
        cwd = $repoRoot
        command = "python trading_mvp/src/premarket_automation.py --repo-root `"$repoRoot`" --tick --attempt-id `"$attemptId`" --worker-handoff-token <redacted> --plan-hash `"$($preflight.plan_hash)`""
        plan_path = $PlanPath
        ...
        private_api = $false
        live_orders = $false
        retry_policy = "failed_or_deferred venue retries on next adaptive interval; scheduler wake is every 5 minutes"
        manifest_path = (Join-Path (Split-Path -Parent $eventsPath) "manifest.json")
    }
    try {
    ...
    Write-JsonFile -Path $launchRecordPath -Payload $launchRecord
        Write-Host "=== Pre-market perpetual listing automation (visible) ===" -ForegroundColor Cyan
        Write-Host ("venues: Bybit + OKX + Gate; cadence stage: " + [string]$state.cadence_stage + "; actual interval: " + [int]$state.cadence_seconds + " sec; scheduler wake: " + $wakeIntervalSec + " sec")
        $env:PYTHONIOENCODING = "utf-8"
        $env:PYTHONUTF8 = "1"
        $workerOutput = & $pythonExe $automationPy --repo-root $repoRoot --tick --attempt-id $attemptId --worker-handoff-token ([string]$pythonHandoff.token) --plan-hash ([string]$preflight.plan_hash) --max-contracts-per-venue 25 --timeout-sec 10 --websocket-duration-sec 10 2>&1 | Out-String
        ...
        if ($workerExit -ne 0 -and [string]$state.status -notin @("RETRY_NEXT_INTERVAL", "PARTIAL_RETRY_NEXT_INTERVAL")) {
            Set-RetryState -Reason ("python_worker_exit_" + $workerExit) -State $state
            $state = Get-State
        }
        ...
        try { $state = Get-State; Set-RetryState -Reason $reason -State $state } catch { }
...
try {
    ...
    $handoffRunId = "premarket_outer_" + (Get-Date).ToUniversalTime().ToString("yyyyMMddTHHmmssfffffffZ") + "_" + $handoffToken.Substring(0, 8)
    $queuedAt = Get-UtcIso
