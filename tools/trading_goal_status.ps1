...
$strategyBranchDispositionPath = Join-Path $repoRoot "docs\plans\strategy-branch-disposition-20260817-v1.json"
$slowLiquidityExactRecollectStatusHelper = Join-Path $repoRoot "tools\slow_liquidity_exact_recollect_status.ps1"
$slowLiquidityExactRecollectPlanPath = Join-Path $repoRoot "docs\plans\slow-liquidity-history-recollect-planonly-20260813-pagecap-provenance-slotintegrity-v6.json"
$slowLiquidityExactRecollectReadinessPath = Join-Path $repoRoot "docs\agent-log\readiness\one-week-historical-edge-sprint-readiness-20260812-v1.json"
...
$slowLiquidityExactRecollectStatus = Get-SlowLiquidityExactRecollectStatus `
    -Gate $gate `
    -PlanPath $slowLiquidityExactRecollectPlanPath `
    -ReadinessPath $slowLiquidityExactRecollectReadinessPath `
...
$slowLiquidityExactRecollectApprovedAwaitingLaunchGate = [bool]$slowLiquidityExactRecollectStatus.approved_awaiting_launch
$slowLiquidityExactRecollectVisibleLaunchStartingGate = [bool]$slowLiquidityExactRecollectStatus.visible_launch_starting
$slowLiquidityExactRecollectRunningGate = [bool]$slowLiquidityExactRecollectStatus.running
$slowLiquidityExactRecollectReadyForQualityGate = [bool]$slowLiquidityExactRecollectStatus.ready_for_quality
$slowLiquidityExactRecollectQualityCommittingGate = [bool]$slowLiquidityExactRecollectStatus.technical_quality_committing
$slowLiquidityExactRecollectStoppedNoRetryGate = [bool]$slowLiquidityExactRecollectStatus.stopped_incomplete_no_retry
$slowLiquidityExactRecollectQualityAcceptedGate = [bool]$slowLiquidityExactRecollectStatus.quality_accepted_awaiting_identity_approval
$slowLiquidityExactRecollectQualityRejectedGate = [bool]$slowLiquidityExactRecollectStatus.quality_rejected_terminal_no_retry
$gateHistory = if ($rawGate) { $rawGate } else { $gate }
$strategyBranchDisposition = Read-JsonFileOrNull -Path $strategyBranchDispositionPath
...
$slowLiquidityFeatureNormalizerArtifactReady = [bool](
    $slowLiquidityFeatureNormalizerArtifact -and
    [string]$slowLiquidityFeatureNormalizerArtifact.decision -eq "SLOW_LIQUIDITY_FEATURE_NORMALIZER_PLANONLY_READY_FOR_FIXED_REPLAY_VALIDATION"
)
$slowLiquidityFeatureNormalizerArtifactRejected = [bool](
    $slowLiquidityFeatureNormalizerArtifact -and
    [string]$slowLiquidityFeatureNormalizerArtifact.decision -eq "SLOW_LIQUIDITY_FEATURE_NORMALIZER_PLANONLY_REJECTED_INSUFFICIENT_EVENTS"
)
$slowLiquidityFeatureNormalizerArtifactIsV1 = [bool](
    $slowLiquidityFeatureNormalizerArtifact -and
    [string]$slowLiquidityFeatureNormalizerArtifact.fixed_contract.signal.compression_metric -eq "range_width_over_atr_sqrt_lookback"
)
...
$nextAllowedAction = if ($slowLiquidityExactRecollectCheckpointGate) {
    if ($slowLiquidityExactRecollectIntegrityBlockedGate) {
        "Exact slow-liquidity lifecycle integrity is blocked. Repair the binding; do not launch, retry, or run quality."
    } else {
        [string]$slowLiquidityExactRecollectStatus.next_action
    }
} elseif ($gate.status -eq "RUNNING") {
...
} elseif ($slowLiquidityExactRecollectIntegrityBlockedGate) {
    "Exact slow-liquidity recollect binding failed integrity checks. Do not create approval, launch record, output, or collector; repair the immutable PlanOnly/readiness binding first."
} elseif ($slowLiquidityExactRecollectAwaitingApprovalGate) {
    "Exact slow-liquidity page-cap-fix recollect is frozen and valid. Await exact hash-bound approval; preflight only, with no receipt, launch, output, replay, grid, live, API or paper-forward."
...
} elseif ($spotPerpBasisCollectAwaitingApprovalGate) {
    "Spot/perp basis collect approval packet is ready. Await explicit confirmation: 'подтверждаю visible spot-perp basis snapshot collect' before starting visible snapshot collector; no replay/grid/live/API/paper-forward."
} elseif ($spotPerpBasisProbeAcceptedGate) {
    "Spot/perp basis public probe accepted (6 paired assets verified). Next step is building the snapshot collect approval packet; no collect/grid/replay/live/API/paper-forward."
...
$primaryEdgeCandidate = if ($slowLiquidityExactRecollectCheckpointGate) {
    if ($slowLiquidityExactRecollectPhase -eq "AWAITING_EXACT_APPROVAL") {
        "Slow liquidity exact recollect awaiting exact approval"
    } else {
        "Slow liquidity exact recollect: $slowLiquidityExactRecollectPhase"
    }
} elseif ($spotPitEventForwardApprovalReadyGate) {
    "MEXC/Gate spot PIT idiosyncratic crash-reclaim forward evidence collect"
} else {
    $null
}
$primaryEdgeStatus = if ($slowLiquidityExactRecollectCheckpointGate) {
    $slowLiquidityExactRecollectResolution
} elseif ($spotPitEventForwardApprovalReadyGate) {
    "spot_pit_event_forward_approval_packet_ready_awaiting_explicit_visible_confirmation"
} else {
    $null
}
...
$result = [ordered]@{
    ...
    slow_liquidity_exact_recollect_checkpoint_gate = $slowLiquidityExactRecollectCheckpointGate
    slow_liquidity_exact_recollect_phase = $slowLiquidityExactRecollectPhase
    ...
    slow_liquidity_exact_recollect_approved_awaiting_launch_gate = $slowLiquidityExactRecollectApprovedAwaitingLaunchGate
    slow_liquidity_exact_recollect_visible_launch_starting_gate = $slowLiquidityExactRecollectVisibleLaunchStartingGate
    slow_liquidity_exact_recollect_running_gate = $slowLiquidityExactRecollectRunningGate
    slow_liquidity_exact_recollect_ready_for_quality_gate = $slowLiquidityExactRecollectReadyForQualityGate
    slow_liquidity_exact_recollect_quality_committing_gate = $slowLiquidityExactRecollectQualityCommittingGate
    slow_liquidity_exact_recollect_stopped_no_retry_gate = $slowLiquidityExactRecollectStoppedNoRetryGate
    slow_liquidity_exact_recollect_quality_accepted_gate = $slowLiquidityExactRecollectQualityAcceptedGate
    slow_liquidity_exact_recollect_quality_rejected_gate = $slowLiquidityExactRecollectQualityRejectedGate
    slow_liquidity_exact_recollect_standing_research_authorized = [bool]$slowLiquidityExactRecollectStatus.standing_research_authorized
    ...
    slow_liquidity_exact_recollect_standing_research_continue_allowed = [bool]$slowLiquidityExactRecollectStatus.standing_research_continue_allowed
    ...
    slow_liquidity_exact_recollect_requires_user_approval = [bool]$slowLiquidityExactRecollectStatus.requires_user_approval
    slow_liquidity_exact_recollect_required_user_input = [string]$slowLiquidityExactRecollectStatus.required_user_input
    ...
    slow_liquidity_feature_normalizer_artifact_ready = $slowLiquidityFeatureNormalizerArtifactReady
    slow_liquidity_feature_normalizer_artifact_rejected = $slowLiquidityFeatureNormalizerArtifactRejected
    ...
    slow_liquidity_feature_normalizer_artifact_path = if ($slowLiquidityFeatureNormalizerArtifact) { [string]$gateHistory.last_slow_liquidity_feature_normalizer_output_path } else { "" }
    strategy_branch_disposition = $strategyBranchDisposition
    ...
    slow_liquidity_fixed_compression_v1_plan_command = $slowLiquidityFixedCompressionV1PlanCommand
    slow_liquidity_fixed_compression_v1_plan_update_gate_command = $slowLiquidityFixedCompressionV1PlanUpdateGateCommand
    ...
    slow_liquidity_exact_recollect_preflight_command = $slowLiquidityExactRecollectPreflightCommand
    slow_liquidity_exact_recollect_approval_packet_command = $slowLiquidityExactRecollectApprovalPacketCommand
    slow_liquidity_exact_recollect_primary_command = $slowLiquidityExactRecollectPrimaryCommand
    slow_liquidity_exact_recollect_launch_command = [string]$slowLiquidityExactRecollectStatus.launch_command
    slow_liquidity_exact_recollect_status_command = [string]$slowLiquidityExactRecollectStatus.status_command
    slow_liquidity_exact_recollect_stop_command = [string]$slowLiquidityExactRecollectStatus.stop_command
    slow_liquidity_exact_recollect_quality_preflight_command = [string]$slowLiquidityExactRecollectStatus.quality_preflight_command
    slow_liquidity_exact_recollect_quality_command = [string]$slowLiquidityExactRecollectStatus.quality_command
...
if (-not [string]::IsNullOrWhiteSpace([string]$primaryEdgeCandidate)) {
    $result.primary_edge_candidate = $primaryEdgeCandidate
    $result.primary_edge_status = $primaryEdgeStatus
}
