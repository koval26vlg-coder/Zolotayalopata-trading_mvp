...
$runtimeModule = "trading_mvp.src.slow_liquidity_official_currentness_topology_v2"
...
$runId = "slow_liquidity_official_currentness_topology_discovery_20260814_v2"
...
if (-not $RuntimeManifestPath) {
    $RuntimeManifestPath = Join-Path $repoRoot `
        "docs\plans\slow-liquidity-official-currentness-topology-runtime-manifest-20260814-v2.json"
}
if (-not $ExecutionManifestPath) {
    $ExecutionManifestPath = Join-Path $repoRoot `
        "docs\plans\slow-liquidity-official-currentness-topology-execution-manifest-20260814-v2.json"
}
if (-not $OutputPath) {
    $OutputPath = `
...
from trading_mvp.src import slow_liquidity_official_currentness_topology_v2 as runtime
...
print(json.dumps({
    "status": "VALID_EXACT_TOPOLOGY_EXECUTION_V2",
...
function Invoke-OfflinePreflight {
    ...
    $raw = @(& $python -m $runtimeModule preflight `
        --repo-root $repoRoot `
        --proposal (Join-Path $repoRoot "docs\plans\drafts\slow-liquidity-official-currentness-topology-v2-refreeze-proposal-20260814-v2.json") `
    ...
    if (
        [string]$payload.status -ne "BLOCKED_AWAIT_EXACT_V2_TOPOLOGY_EXECUTION_APPROVAL" -or
...
function Invoke-FullPreflight {
    ...
    if (-not (Test-Path -LiteralPath $ExecutionManifestPath -PathType Leaf)) {
        throw "exact_v2_execution_manifest_missing"
    }
    ...
    if ([string]$validation.status -ne "VALID_EXACT_TOPOLOGY_EXECUTION_V2" -or [string]$validation.run_id -ne $runId -or [string]$validation.output_path -ne [System.IO.Path]::GetFullPath($OutputPath)) {
    ...
    if ([string]$guard.status -ne "ACTIVE" -or [string]$guard.decision -ne "RUN_SLOW_LIQUIDITY_OFFICIAL_CURRENTNESS_TOPOLOGY_DISCOVERY_V2" -or [string]$guard.usage.status -ne "AVAILABLE" -or [string]$guard.usage.decision -ne "CONTINUE" -or [double]$guard.usage.remaining_percent -le 15) {
        throw "authoritative_guard_did_not_authorize_exact_topology_run"
    }
    $gateRaw = @(& $activeRunChecker 2>&1)
...
if ($Status) {
    [ordered]@{
        schema = "trading_mvp_slow_liquidity_official_topology_status_v2"
        run_id = $runId
...
if (-not $VisibleWorker) {
    ...
    [ordered]@{
        schema = "trading_mvp_slow_liquidity_official_topology_visible_launch_v2"
...
if ([string]$preflight.status -ne "READY_FOR_VISIBLE_SINGLE_USE_TOPOLOGY_RUN") {
    ...
    schema = "trading_mvp_slow_liquidity_official_topology_launch_v2"
...
try {
    ...
    $capability = [ordered]@{
        schema = "trading_mvp_slow_liquidity_official_topology_launcher_capability_v2"
        status = "ACTIVE"
        run_id = $runId
        owner_pid = $PID
