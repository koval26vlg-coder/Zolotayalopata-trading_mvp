param(
    [string]$SourcePlanPath = "E:\ZolotyayLopata-data\exports\trading-mvp\fast-edge-v5\plans\fast_first_wick_rejection_reversal_planonly_20260714_140353.json",
...
if ($MaxRuntimeSec -lt 1 -or $MaxRuntimeSec -gt 1200) {
    throw "MaxRuntimeSec must be between 1 and 1200 for Fast-First v6 PlanOnly."
}
...
$ModulePath = Join-Path $RepoRoot "trading_mvp\src\weekend_liquidity_window.py"
$Stamp = Get-Date -Format "yyyyMMdd_HHmmss"
$RunId = "fast_first_v6_weekend_liquidity_planonly_$Stamp"

if ([string]::IsNullOrWhiteSpace($OutputPath)) {
    $OutputPath = "E:\ZolotyayLopata-data\exports\trading-mvp\fast-edge-v6\plans\fast_first_weekend_liquidity_window_planonly_$Stamp.json"
}
...
Write-Host "[3/4] Hash verification and v6 PlanOnly freeze" -ForegroundColor Cyan
...
if ($LASTEXITCODE -ne 0) {
    throw "Fast-First v6 PlanOnly build failed with exit code $LASTEXITCODE."
}

Write-Host "[4/4] Independent persisted-artifact validation" -ForegroundColor Cyan
& $Python $ModulePath validate --plan $OutputPath
if ($LASTEXITCODE -ne 0) {
    throw "Fast-First v6 PlanOnly validation failed with exit code $LASTEXITCODE."
}
...
$ManifestRecord = [ordered]@{
    schema = "fast_first_v6_planonly_manifest_v1"
    run_id = $RunId
    final = $true
    completed_cycles = 1
    cycles = 1
...
$GateArchive = Join-Path $ArchiveDir "active-run-gate.$SafePriorRunId.superseded-by-v6-planonly.$ArchiveStamp.json"
$PointerArchive = Join-Path $ArchiveDir "current-run.$SafePriorRunId.superseded-by-v6-planonly.$ArchiveStamp.json"
...
$LaunchRecord = [ordered]@{
    schema = "fast_first_v6_planonly_record_v1"
...
$GateRecord = [ordered]@{
    ...
    output = [ordered]@{
        path = $ResolvedOutput
        type = "fast_first_v6_weekend_liquidity_window_planonly"
    ...
    hypothesis_id = [string]$Plan.hypothesis.id
    plan_state = "PLAN_FROZEN_OOS_NOT_EVALUATED"
    purpose = "Freeze final Fast-First v6 weekend-liquidity calendar window before any train/OOS performance access."
    stop_reason = "completed_planonly_freeze"
    next_goal_decision = "FAST_FIRST_V6_PLAN_FROZEN"
    next_goal_reason = "Final independent weekend-liquidity calendar-window contract, data seal, costs, split, folds and gates are frozen without OOS metrics."
    next_step_after_ready = "Implement and test the hash-bound no-grid v6 evaluator. Do not run OOS until evaluator readiness is validated."
    raw_gate_next_step_after_ready = "Implement and test the hash-bound no-grid v6 evaluator. Do not run OOS until evaluator readiness is validated."
