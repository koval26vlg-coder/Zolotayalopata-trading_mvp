...
$runtimeModule = "trading_mvp.src.slow_liquidity_identity_request_plan_discovery_v3"
...
$runId = "slow_liquidity_identity_request_plan_discovery_20260815_v3"
...
$parentRuntimeManifestPath = Join-Path $repoRoot `
    "docs\plans\slow-liquidity-identity-request-plan-discovery-20260813-v2\runtime-manifest.json"
$parentLaunchRecordPath = Join-Path $repoRoot `
    "docs\agent-log\run-gates\slow_liquidity_official_currentness_topology_discovery_20260814_v4.launch.json"
...
if (-not $RuntimeManifestPath) {
    $RuntimeManifestPath = Join-Path $repoRoot `
        "docs\plans\slow-liquidity-identity-request-plan-discovery-runtime-manifest-20260815-v3-r2.json"
}
if (-not $ExecutionManifestPath) {
    $ExecutionManifestPath = Join-Path $repoRoot `
        "docs\plans\slow-liquidity-identity-request-plan-discovery-execution-manifest-20260815-v3.json"
}
if (-not $OutputPath) {
    $OutputPath = `
        "E:\ZolotyayLopata-data\exports\trading-mvp\slow-liquidity-identity-request-plan\$runId"
}
...
function Write-JsonCreateNew {
    ...
    try {
        ...
        throw "Exact request-plan discovery launch record already exists; duplicate launch is forbidden."
    }
    try {
...
from trading_mvp.src import slow_liquidity_identity_request_plan_discovery_v3 as runtime
...
print(json.dumps({
    "status": "VALID_EXACT_REQUEST_PLAN_DISCOVERY_EXECUTION_V3",
...
'@
    ...
    if ($LASTEXITCODE -ne 0) { throw "Request-plan discovery execution validation failed." }
...
try:
    ...
    print(json.dumps({
        ...
        "request_plan_output_created": True,
        "identity_evidence_created": False,
        "request_plan_created": True,
...
'@
    ...
    if (-not $process.Start()) {
        $process.Dispose()
        throw "Request-plan discovery runtime process did not start."
    }
    return $process
}
...
function Get-SanitizedLauncherFailureCode {
    ...
    $allowlist = @(
        ...
        "REQUEST_PLAN_DISCOVERY_RUNTIME_DEADLINE_EXCEEDED",
        "RESPONSE_CAP_EXCEEDED",
        "REQUEST_PLAN_DISCOVERY_RUNTIME_CONTRACT_REJECTED",
        "REQUEST_PLAN_DISCOVERY_INTERNAL_RUNTIME_FAILURE"
    )
    if ($allowlist -contains $Message) {
        return $Message
    }
    if ($Message -eq "Request-plan discovery runtime exceeded the exact 600 second cap.") {
        return "REQUEST_PLAN_DISCOVERY_RUNTIME_DEADLINE_EXCEEDED"
    }
...
function Invoke-OfflinePreflight {
    ...
    if (
        [string]$payload.status -ne "BLOCKED_AWAIT_EXACT_REQUEST_PLAN_DISCOVERY_V3_EXECUTION_APPROVAL" -or
        [string]$payload.run_id -ne $runId -or
        [bool]$payload.network_accessed -or
        [bool]$payload.execution_manifest_read -or
        [bool]$payload.request_plan_output_created -or
...
function Invoke-FullPreflight {
    ...
    if (-not (Test-Path -LiteralPath $ExecutionManifestPath -PathType Leaf)) {
        throw "exact_request_plan_discovery_v3_execution_manifest_missing"
    }
    ...
    if ([string]$validation.status -ne "VALID_EXACT_REQUEST_PLAN_DISCOVERY_EXECUTION_V3" -or [string]$validation.run_id -ne $runId -or [string]$validation.output_path -ne [System.IO.Path]::GetFullPath($OutputPath)) {
    ...
    if ($now.AddSeconds(600) -gt $hardDeadline) { throw "hard_deadline_runtime_budget_unavailable" }
    ...
    if ([string]$guard.status -ne "ACTIVE" -or [string]$guard.decision -ne "RUN_SLOW_LIQUIDITY_IDENTITY_REQUEST_PLAN_DISCOVERY_V3" -or [string]$guard.usage.status -ne "AVAILABLE" -or [string]$guard.usage.decision -ne "CONTINUE" -or [double]$guard.usage.remaining_percent -le 15) {
        throw "authoritative_guard_did_not_authorize_exact_request_plan_discovery_run"
    }
    ...
    return [ordered]@{
        status = if ($reasons.Count -eq 0) { "READY_FOR_VISIBLE_SINGLE_USE_REQUEST_PLAN_DISCOVERY_RUN" } else { "BLOCKED" }
        reasons = @($reasons)
        run_id = $runId
...
if ($Status) {
    [ordered]@{
        schema = "trading_mvp_slow_liquidity_identity_request_plan_discovery_status_v3"
        run_id = $runId
...
if ($Stop) {
    if (-not (Test-Path -LiteralPath $launchRecordPath -PathType Leaf)) {
        throw "Exact request-plan discovery launch record is missing; there is no owned run to stop."
    }
    $record = Read-JsonFile -Path $launchRecordPath
    if ([string]$record.run_id -ne $runId) {
        throw "Request-plan discovery launch record run_id mismatch."
    }
    ...
    if (
        ...
        throw "Global writer claim does not match the visible request-plan discovery owner."
    }
    ...
    if ($ownerProcess) {
        ...
        if (
            ...
            throw "Visible request-plan discovery owner PID was reused; refusing stop."
        }
    }
...
if ($PreflightOnly) {
    try {
        ...
        [ordered]@{
            status = "BLOCKED_AWAIT_EXACT_REQUEST_PLAN_DISCOVERY_V3_EXECUTION_APPROVAL"
...
if (-not $VisibleWorker) {
    ...
    if ([string]$preflight.status -ne "READY_FOR_VISIBLE_SINGLE_USE_REQUEST_PLAN_DISCOVERY_RUN") {
        throw "Exact request-plan discovery run is not authorized: $($preflight.status); $($preflight.reasons -join ',')."
    }
    ...
    while ([DateTimeOffset]::UtcNow -lt $deadline) {
        if ($terminal.HasExited) {
            throw "Visible request-plan discovery terminal exited before claiming the exact run."
        }
    ...
    if (-not $record) {
        throw "Visible request-plan discovery terminal did not claim the exact run within 30 seconds."
    }
    [ordered]@{
        schema = "trading_mvp_slow_liquidity_identity_request_plan_discovery_visible_launch_v3"
...
if ([string]$preflight.status -ne "READY_FOR_VISIBLE_SINGLE_USE_REQUEST_PLAN_DISCOVERY_RUN") {
    ...
    schema = "trading_mvp_slow_liquidity_identity_request_plan_discovery_launch_v3"
...
try {
    ...
    $recordOwned = $true
    Write-Host "[slow-liquidity-request-plan] exact visible worker claimed: $runId" `
    ...
    $claim = Invoke-WriterClaim -Arguments @(
        ...
        "--owner-kind", "slow_liquidity_identity_request_plan_discovery",
    ...
    if ([int]$writerTopology.parent_process_id -ne $PID) {
        throw "Request-plan discovery runtime is not owned by the visible terminal."
    }
    ...
    $capability = [ordered]@{
        schema = "trading_mvp_slow_liquidity_identity_request_plan_discovery_launcher_capability_v3"
        status = "ACTIVE"
        run_id = $runId
        owner_pid = $PID
    ...
    while (-not $runtimeProcess.HasExited) {
        if ([DateTimeOffset]::UtcNow -ge $runtimeDeadline) {
            ...
            throw "Request-plan discovery runtime exceeded the exact 600 second cap."
        }
        if ([DateTimeOffset]::UtcNow -ge $nextProgress) {
            ...
            Write-Host "[slow-liquidity-request-plan] RUNNING elapsed_sec=$elapsed writer_pid=$($runtimeProcess.Id)"
            $nextProgress = [DateTimeOffset]::UtcNow.AddSeconds(5)
        }
    ...
    if ($runtimeProcess.ExitCode -ne 0) {
        $controlledReason = "REQUEST_PLAN_DISCOVERY_INTERNAL_RUNTIME_FAILURE"
        ...
        try {
            ...
            $launchRecord.request_plan_output_created = $failure.request_plan_output_created
        } catch {
    ...
    $launchRecord.request_plan_output_created = [bool]$runtimePayload.request_plan_output_created
    ...
    Write-Host "[slow-liquidity-request-plan] terminal result: $($runtimePayload.status)" `
    ...
    Write-Host "[slow-liquidity-request-plan] STOPPED_INCOMPLETE; retry is not authorized." `
        -ForegroundColor Red
    throw
} finally {
    ...
    if ($claimToken) {
        try {
            ...
            Write-Host "[slow-liquidity-request-plan] writer claim release failed: $($_.Exception.Message)" `
                -ForegroundColor Red
        }
    }
}
