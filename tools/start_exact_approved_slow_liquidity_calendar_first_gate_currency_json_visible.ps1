...
$launcherFileName = "start_exact_approved_slow_liquidity_calendar_first_gate_currency_json_visible.ps1"
$runtimeModule = Join-Path $repoRoot "trading_mvp\src\slow_liquidity_calendar_first_gate_currency_json.py"
...
$planPath = Join-Path $repoRoot "docs\plans\slow-liquidity-calendar-first-gate-currency-json-planonly-20260816.json"
...
$runId = "slow_liquidity_calendar_first_gate_currency_json_20260816"
$launchRecordPath = Join-Path $repoRoot "docs\agent-log\run-gates\$runId.launch.json"
$outputPath = "E:\ZolotyayLopata-data\exports\trading-mvp\slow-liquidity-calendar-first-gate-currency-json\$runId"
$expectedPlanHash = "388d81d40e866eb7ccfd93cc439c27a9c220e3f5167eb62cc92d6bbc1983f756"
$expectedPlanFileSha256 = "25ab519895f262f6482a5b4c1b107f1517c83eadfdda235f4d8fac0f3414e1fd"
$maxRuntimeSec = 900
...
function Test-VisibleConsoleWindow {
    ...
    if ($null -eq ("TradingMvp.CalendarFirstGateCurrencyJsonVisibleConsoleNative" -as [type])) {
        Add-Type -TypeDefinition @'
using System;
using System.Runtime.InteropServices;

namespace TradingMvp {
    public static class CalendarFirstGateCurrencyJsonVisibleConsoleNative {
...
'@
    ...
    $consoleWindow = [TradingMvp.CalendarFirstGateCurrencyJsonVisibleConsoleNative]::GetConsoleWindow()
    return (
        $consoleWindow -ne [IntPtr]::Zero -and
        [TradingMvp.CalendarFirstGateCurrencyJsonVisibleConsoleNative]::IsWindowVisible($consoleWindow)
    )
}
...
function Assert-FrozenPlan {
    if ((Get-Sha256 -Path $planPath) -cne $expectedPlanFileSha256) {
        throw "Frozen calendar-first Gate currency JSON plan file SHA256 mismatch."
    }
    ...
    if (
        ...
        [bool]$plan.identity_verdict_allowed -or
        [bool]$plan.ohlcv_collect_authorized -or
        [bool]$plan.spot_v2_runtime_reuse -or
        [int]$plan.selected_base_count -ne 407 -or
        [string]$plan.mode -cne "PlanOnly"
    ) {
        throw "Frozen calendar-first Gate currency JSON plan identity mismatch."
    }
    $raw = Get-Content -Raw -LiteralPath $planPath
    if (
        $raw -cmatch "20260815-v7" -or
        $raw -cmatch "contract\.mexc\.com" -or
        $raw -cmatch "\{BASE\}_USDT" -or
        $raw -cmatch "www\.bing\.com" -or
        $raw -cmatch "sitemap-index" -or
        $raw -cnotmatch "api\.gateio\.ws/api/v4/spot/currencies/" -or
        $raw -cnotmatch '"not_html_official_page_request_plan": true'
    ) {
        throw "Frozen calendar-first Gate currency JSON plan leaked locator, v7, or perp bindings."
    }
    return $plan
}
...
function Invoke-GuardPreflight {
    ...
    if ([string]$autopilot.status -notin @("ACTIVE", "WAITING_EVENT")) {
        throw "Autopilot guard is not ACTIVE: $($autopilot.status)"
    }
    $v4NoRetry = [string]$autopilot.decision -eq
        "TERMINAL_REJECT_SLOW_LIQUIDITY_IDENTITY_REQUEST_PLAN_DISCOVERY_V4_STOPPED_INCOMPLETE_NO_RETRY"
    if ($autopilot.stop_new_actions -ne $false -and -not $v4NoRetry) {
...
function Invoke-FullPreflight {
    ...
    if ((Split-Path -Leaf $PSCommandPath) -ne $launcherFileName) {
        throw "The calendar-first Gate currency JSON run must use its exact top-level launcher."
    }
    foreach ($required in @(
    ...
    if (Test-Path -LiteralPath (Join-Path $outputPath "gate-currency-records.json") -PathType Leaf) {
        throw "Immutable calendar-first Gate currency JSON output already exists."
    }
    if (Test-Path -LiteralPath (Join-Path $outputPath "manifest.json") -PathType Leaf) {
        throw "Immutable calendar-first Gate currency JSON manifest already exists."
    }
    if (-not $IgnoreOwnLaunchRecord) {
        $existing = Get-ExistingLaunchDisposition
        if ($existing) {
            return [ordered]@{
                schema = "trading_mvp_calendar_first_gate_currency_json_preflight_v1"
...
function New-GlobalWriterClaim {
    ...
    $raw = @(& $python $writerClaimCli `
        ...
        "--owner-kind" "slow_liquidity_calendar_first_gate_currency_json" `
...
function Assert-CompletedOutput {
    $recordsPath = Join-Path $outputPath "gate-currency-records.json"
    $manifestPath = Join-Path $outputPath "manifest.json"
    if (-not (Test-Path -LiteralPath $recordsPath -PathType Leaf)) {
        throw "gate-currency-records.json is missing."
    }
    if (-not (Test-Path -LiteralPath $manifestPath -PathType Leaf)) {
        throw "calendar-first Gate currency JSON manifest.json is missing."
    }
    ...
    if ([bool]$manifest.identity_verdict -or [bool]$manifest.identity_evidence) {
        throw "calendar-first Gate currency JSON output claimed an identity verdict."
    }
    if ([bool]$manifest.html_request_plan -or [bool]$manifest.page_locator_used -or [bool]$manifest.bing_used) {
        throw "calendar-first Gate currency JSON output claimed an HTML page locator."
    }
    if ([bool]$manifest.v7_used -or [bool]$manifest.mexc_json_used -or [bool]$manifest.spot_v2_runtime_reuse) {
        throw "calendar-first Gate currency JSON output claimed v7, MEXC JSON, or spot v2 reuse."
    }
    if ([bool]$manifest.ohlcv_collect) {
        throw "calendar-first Gate currency JSON output claimed OHLCV collect."
    }
    if ([string]$manifest.plan_hash -cne $expectedPlanHash) {
        throw "calendar-first Gate currency JSON output plan_hash mismatch."
    }
    if ([string]$manifest.status -ne "CALENDAR_FIRST_GATE_CURRENCY_JSON_INCOMPLETE") {
        throw "calendar-first Gate currency JSON output status is not an authorized terminal status."
    }
    ...
    if ($totalBytes -gt $hardOutputCapBytes) {
        throw "calendar-first Gate currency JSON output exceeds the 20 MB cap."
    }
    return [ordered]@{
...
if ($Status) {
    [ordered]@{
        schema = "trading_mvp_calendar_first_gate_currency_json_status_v1"
        run_id = $runId
...
if ($Stop) {
    if (-not (Test-Path -LiteralPath $launchRecordPath -PathType Leaf)) {
        throw "Exact calendar-first Gate currency JSON launch record is missing; there is no owned run to stop."
    }
    $record = ConvertFrom-JsonPreserveDateStrings -InputJson (
        Get-Content -Raw -LiteralPath $launchRecordPath
    )
...
if (-not $VisibleWorker) {
    ...
    if ([string]$preflight.status -ne "READY_FOR_VISIBLE_SINGLE_USE") {
        throw "Visible calendar-first Gate currency JSON preflight did not authorize the single-use run."
    }
    ...
    while ([DateTimeOffset]::UtcNow -lt $deadline) {
        if ($terminal.HasExited) {
            throw "Visible calendar-first Gate currency JSON terminal exited before claiming the exact run."
        }
    ...
    if (-not $ownedRecord) {
        throw "Visible calendar-first Gate currency JSON terminal did not claim the exact run within 30 seconds."
    }
    [ordered]@{
        schema = "trading_mvp_calendar_first_gate_currency_json_visible_launch_v1"
...
$script:launchRecord = [ordered]@{
    schema = "trading_mvp_calendar_first_gate_currency_json_launch_v1"
    ...
    message = "Visible worker claimed the exact single-use calendar-first Gate currency JSON run."
}
...
try {
    ...
    Write-Host "[calendar-first-gate-json] visible exact worker claimed: $runId" -ForegroundColor Cyan
    ...
    Write-Host "[calendar-first-gate-json] running; pid=$($runtimeProcess.Id); cap=${maxRuntimeSec}s" -ForegroundColor Green
    ...
    while (-not $runtimeProcess.HasExited) {
        if ($stopwatch.Elapsed.TotalSeconds -ge $maxRuntimeSec) {
            ...
            throw "calendar-first Gate currency JSON runtime exceeded the exact 900-second cap."
        }
        ...
        Write-Host ("[calendar-first-gate-json] elapsed={0:n1}s writer_pid={1}" -f $stopwatch.Elapsed.TotalSeconds, $runtimeProcess.Id)
    ...
    if ($runtimeProcess.ExitCode -notin @(0, 2)) {
        throw "calendar-first Gate currency JSON runtime failed with exit code $($runtimeProcess.ExitCode)."
    }
    ...
    Write-Host "[calendar-first-gate-json] $($completed.status); identity_verdict=false; retry=false" -ForegroundColor Green
} catch {
    ...
    Write-Host "[calendar-first-gate-json] STOPPED_INCOMPLETE; retry is not authorized." -ForegroundColor Red
    throw
} finally {
    if ($globalClaimToken) {
        try {
            ...
            Write-Host "[calendar-first-gate-json] writer claim release failed: $($_.Exception.Message)" -ForegroundColor Red
        }
    }
}
