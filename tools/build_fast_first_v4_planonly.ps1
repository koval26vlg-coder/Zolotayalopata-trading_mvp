param(
    [string]$SourcePlanPath = "E:\ZolotyayLopata-data\exports\trading-mvp\fast-edge-v3\plans\fast_first_lottery_max_planonly_20260713.json",
...
# Invocation accepts -MaxRuntimeSec, but never permits a PlanOnly build over 20 minutes.
if ($MaxRuntimeSec -lt 1 -or $MaxRuntimeSec -gt 1200) {
    throw "MaxRuntimeSec must be between 1 and 1200 for Fast-First v4 PlanOnly."
}
...
$ModulePath = Join-Path $RepoRoot "trading_mvp\src\funding_pressure_reversal.py"
$Stamp = Get-Date -Format "yyyyMMdd_HHmmss"
$RunId = "fast_first_v4_funding_pressure_reversal_planonly_$Stamp"
$OutputPathProvided = -not [string]::IsNullOrWhiteSpace($OutputPath)

if ($RegisterExisting -and -not $OutputPathProvided) {
    throw "OutputPath is required with RegisterExisting."
}
if (-not $OutputPathProvided) {
    $OutputPath = "E:\ZolotyayLopata-data\exports\trading-mvp\fast-edge-v4\plans\fast_first_funding_pressure_reversal_planonly_$Stamp.json"
}
...
if ($Gate.status -eq "STOPPED_INCOMPLETE") {
    throw "Active-run gate is STOPPED_INCOMPLETE. Resolve or explicitly reject the incomplete run first."
}
if ($Gate.status -ne "READY_FOR_POSTPROCESS") {
    throw "Unexpected active-run gate status: $($Gate.status)"
}
...
if ($RegisterExisting -and -not (Test-Path -LiteralPath $OutputPath -PathType Leaf)) {
    throw "Existing PlanOnly artifact does not exist: $OutputPath"
}
if (-not $RegisterExisting -and (Test-Path -LiteralPath $OutputPath)) {
...
foreach ($Candidate in @(
    ...
    if (-not $Python -and (Test-Path -LiteralPath $Candidate -PathType Leaf)) {
        $Python = $Candidate
    }
}
if (-not $Python) {
    throw "Python runtime was not found in PATH or the supported installation paths."
}
$StartedAt = Get-Date
...
if ($RegisterExisting) {
    Write-Host "[3/4] REGISTER_EXISTING_PLANONLY: build skipped" -ForegroundColor Cyan
} else {
    Write-Host "[3/4] Hash verification and PlanOnly freeze" -ForegroundColor Cyan
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Fast-First v4 PlanOnly build failed with exit code $LASTEXITCODE."
    }
}
...
if ($LASTEXITCODE -ne 0) {
    throw "Fast-First v4 PlanOnly validation failed with exit code $LASTEXITCODE."
}
...
$ManifestRecord = [ordered]@{
    schema = "fast_first_v4_planonly_manifest_v1"
    run_id = $RunId
    final = $true
    completed_cycles = 1
    cycles = 1
...
$GateArchive = Join-Path $ArchiveDir "active-run-gate.$SafePriorRunId.superseded-by-v4-planonly.$ArchiveStamp.json"
$PointerArchive = Join-Path $ArchiveDir "current-run.$SafePriorRunId.superseded-by-v4-planonly.$ArchiveStamp.json"
...
$LaunchRecord = [ordered]@{
    schema = "fast_first_v4_planonly_record_v1"
    mode = if ($RegisterExisting) { "REGISTER_EXISTING_PLANONLY" } else { "VISIBLE_BOUNDED_PLANONLY" }
    run_id = $RunId
    process_started = -not $RegisterExisting
...
$GateRecord = [ordered]@{
    ...
    output = [ordered]@{
        path = $ResolvedOutput
        type = "fast_first_v4_funding_pressure_reversal_planonly"
    ...
    hypothesis_id = [string]$Plan.hypothesis.id
    plan_state = "PLAN_FROZEN_OOS_NOT_EVALUATED"
    purpose = "Freeze Fast-First v4 before any train/OOS performance access."
    stop_reason = "completed_planonly_freeze"
    next_goal_decision = "FAST_FIRST_V4_PLAN_FROZEN"
    next_goal_reason = "Funding-pressure reversal contract, data seal, costs, split, folds and gates are frozen without OOS metrics."
    next_step_after_ready = "Implement and test the hash-bound no-grid evaluator. Do not run OOS yet."
    raw_gate_next_step_after_ready = "Implement and test the hash-bound no-grid evaluator. Do not run OOS yet."
