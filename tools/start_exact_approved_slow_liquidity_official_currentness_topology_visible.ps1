...
$runtimeModule = "trading_mvp.src.slow_liquidity_official_currentness_topology"
...
$runId = "slow_liquidity_official_currentness_topology_discovery_20260813_v1"
...
if (-not $RuntimeManifestPath) {
    $RuntimeManifestPath = Join-Path $repoRoot `
        "docs\plans\slow-liquidity-official-currentness-topology-runtime-manifest-20260813-v1.json"
}
if (-not $ExecutionManifestPath) {
    $ExecutionManifestPath = Join-Path $repoRoot `
        "docs\plans\slow-liquidity-official-currentness-topology-execution-manifest-20260814-v1.json"
}
if (-not $OutputPath) {
    $OutputPath = `
...
from trading_mvp.src import slow_liquidity_official_currentness_topology as runtime
...
print(json.dumps({"status":"VALID_EXACT_TOPOLOGY_EXECUTION","run_id":capability.run_id,"runtime_manifest_hash":capability.runtime_manifest_hash,"execution_manifest_hash":capability.execution_manifest_hash,"output_path":capability.output_path}))
'@
...
def emit_failure(reason):
    safe_reasons = {
        "HTTP redirect is forbidden": "HTTP_REDIRECT_FORBIDDEN",
        "official topology HTTP request failed": "OFFICIAL_TOPOLOGY_HTTP_REQUEST_FAILED",
        "topology runtime deadline exceeded": "TOPOLOGY_RUNTIME_DEADLINE_EXCEEDED",
        "response cap exceeded": "RESPONSE_CAP_EXCEEDED",
        "content length exceeds response cap": "CONTENT_LENGTH_EXCEEDS_RESPONSE_CAP",
        "compressed response encoding is forbidden": "COMPRESSED_RESPONSE_ENCODING_FORBIDDEN",
        "official source must return HTTP 200": "OFFICIAL_SOURCE_HTTP_STATUS_NOT_200",
        "TOPOLOGY_INTERNAL_RUNTIME_FAILURE": "TOPOLOGY_INTERNAL_RUNTIME_FAILURE",
    }
    reason_code = safe_reasons.get(reason, "TOPOLOGY_RUNTIME_CONTRACT_REJECTED")
    print(json.dumps({
        "reason": reason_code,
...
try:
    ...
    emit_failure("TOPOLOGY_INTERNAL_RUNTIME_FAILURE")
    raise SystemExit(3)
'@
...
function Invoke-FullPreflight {
    Assert-ExpectedFileHash -Path $RuntimeManifestPath -Expected "52abef8c08b0d928758f62fc093a402c4954a6df7470b27cf4d1b5b4d9abf4b8" -Label "Runtime manifest"
    ...
    Assert-ExpectedFileHash -Path $ExecutionManifestPath -Expected "a5a8413ca46ed6aa92a7462e02185729c32337c00fbbcf7608ccfafb57c0f747" -Label "Execution manifest"
    ...
    if ([string]$validation.status -ne "VALID_EXACT_TOPOLOGY_EXECUTION" -or [string]$validation.run_id -ne $runId -or [string]$validation.output_path -ne [System.IO.Path]::GetFullPath($OutputPath)) {
        throw "Exact topology execution binding mismatch."
    }
    $now = [DateTimeOffset]::Now
    $latestLaunch = [DateTimeOffset]::Parse("2026-08-14T07:45:00+03:00")
    $hardDeadline = [DateTimeOffset]::Parse("2026-08-14T08:00:00+03:00")
    ...
    if ([string]$guard.status -ne "ACTIVE" -or [string]$guard.decision -ne "RUN_SLOW_LIQUIDITY_OFFICIAL_CURRENTNESS_TOPOLOGY_DISCOVERY" -or [string]$guard.usage.status -ne "AVAILABLE" -or [string]$guard.usage.decision -ne "CONTINUE" -or [double]$guard.usage.remaining_percent -le 15) {
...
if ($Status) {
    [ordered]@{
        schema = "trading_mvp_slow_liquidity_official_topology_status_v1"
        run_id = $runId
...
if ($PreflightOnly) {
    try {
        ...
        if ([string]$preflight.status -eq "READY_FOR_VISIBLE_SINGLE_USE_TOPOLOGY_RUN") {
            exit 0
        }
        exit 2
    } catch {
...
if (-not $VisibleWorker) {
    ...
    [ordered]@{
        schema = "trading_mvp_slow_liquidity_official_topology_visible_launch_v1"
...
if ([string]$preflight.status -ne "READY_FOR_VISIBLE_SINGLE_USE_TOPOLOGY_RUN") {
    ...
    schema = "trading_mvp_slow_liquidity_official_topology_launch_v1"
...
try {
    ...
    $capability = [ordered]@{
        schema = "trading_mvp_slow_liquidity_official_topology_launcher_capability_v1"
        status = "ACTIVE"
        run_id = $runId
        owner_pid = $PID
    ...
    if ($runtimeProcess.ExitCode -ne 0) {
        $controlledReason = "topology_runtime_failed_exit_$($runtimeProcess.ExitCode)"
        try {
            ...
            if ([string]$failure.reason) {
                ...
                $launchRecord.failure_reason_code = [string]$failure.reason
            }
