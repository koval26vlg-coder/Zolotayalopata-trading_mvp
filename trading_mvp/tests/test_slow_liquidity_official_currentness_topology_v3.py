...
from trading_mvp.src import slow_liquidity_official_currentness_topology_v3 as topology


ROOT = Path(__file__).resolve().parents[2]
PROPOSAL_PATH = (
    ROOT / "docs/plans/drafts/slow-liquidity-official-currentness-topology-v3-"
    "refreeze-proposal-20260814-v1.json"
)
RUNTIME_PATH = (
...
RUNTIME_MODULE_PATH = (
    ROOT / "trading_mvp/src/slow_liquidity_official_currentness_topology_v3.py"
)
V2_RUNTIME_PATH = (
    ROOT / "trading_mvp/src/slow_liquidity_official_currentness_topology_v2.py"
)
V2_RUNTIME_MANIFEST_PATH = (
    ROOT / "docs/plans/slow-liquidity-official-currentness-topology-runtime-"
...
INTEGRITY_AUDIT_PATH = (
    ROOT / "docs/agent-log/readiness/slow-liquidity-official-currentness-"
...
V2_RUNTIME_MANIFEST_SHA256 = "05e13fbb0524f0ea3dc7f4d3f6003011037c02e7d922b74454188a9a22db4e94"
V2_LAUNCHER_SHA256 = "4d5ae74e0d3e8169d220b4e4d38ac0c339b8f867b0d368475a552ca64a3937a3"
INTEGRITY_AUDIT_SHA256 = "c919adb83b4e0674aa321fa03d52453930c0c0f81c5babd04ce9ac9bc6f7bbd0"
...
def test_exact_v3_bindings_and_v2_is_immutable() -> None:
    assert topology.RUN_ID.endswith("20260814_v3")
    ...
    v2_manifest = json.loads(V2_RUNTIME_MANIFEST_PATH.read_text(encoding="utf-8"))
    assert _sha256(V2_RUNTIME_MANIFEST_PATH) == V2_RUNTIME_MANIFEST_SHA256
    assert _sha256(V2_RUNTIME_PATH) == v2_manifest["runtime"]["module_sha256"]
    assert _sha256(V2_LAUNCHER_PATH) == V2_LAUNCHER_SHA256
    assert _sha256(INTEGRITY_AUDIT_PATH) == INTEGRITY_AUDIT_SHA256
    assert topology.INTEGRITY_AUDIT_FILE_SHA256 == INTEGRITY_AUDIT_SHA256
...
def test_preflight_reads_no_execution_manifest_and_has_no_side_effects() -> None:
    ...
    assert result["status"] == "BLOCKED_AWAIT_EXACT_V3_TOPOLOGY_EXECUTION_APPROVAL"
...
def test_launcher_is_fail_closed_and_prepares_output_only_after_writer_claim() -> None:
    ...
    assert "slow_liquidity_official_currentness_topology_v3" in launcher
    ...
    assert "BLOCKED_AWAIT_EXACT_V3_TOPOLOGY_EXECUTION_APPROVAL" in launcher
    assert "RUN_SLOW_LIQUIDITY_OFFICIAL_CURRENTNESS_TOPOLOGY_DISCOVERY_V3" in launcher
    ...
    missing_manifest_index = launcher.index("exact_v3_execution_manifest_missing")
...
def test_launcher_preflight_and_direct_call_create_nothing() -> None:
    ...
    assert payload["status"] == "BLOCKED_AWAIT_EXACT_V3_TOPOLOGY_EXECUTION_APPROVAL"
...
def test_runtime_cli_has_preflight_but_no_execute_command() -> None:
    completed = subprocess.run(
        [
            sys.executable,
            "-m",
            "trading_mvp.src.slow_liquidity_official_currentness_topology_v3",
            "execute",
        ],
        cwd=ROOT,
        capture_output=True,
