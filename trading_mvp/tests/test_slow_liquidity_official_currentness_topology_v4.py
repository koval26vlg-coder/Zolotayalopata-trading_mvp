...
from trading_mvp.src import slow_liquidity_official_currentness_topology_v4 as topology
...
PARENT_LAUNCH_RECORD_PATH = (
    ROOT / "docs/agent-log/run-gates/"
    "slow_liquidity_official_currentness_topology_discovery_20260814_v3.launch.json"
)
PARENT_IDENTITY_RUNTIME_PATH = (
    ROOT / "docs/plans/slow-liquidity-official-identity-runtime-manifest-"
    "20260813-v4.json"
)
RUNTIME_PATH = (
    ROOT / "docs/plans/slow-liquidity-official-currentness-topology-runtime-"
...
RUNTIME_MODULE_PATH = (
    ROOT / "trading_mvp/src/slow_liquidity_official_currentness_topology_v4.py"
)
OUTPUT_PATH = Path(
...
def test_exact_v4_parent_bindings_are_immutable() -> None:
    assert topology.RUN_ID.endswith("20260814_v4")
    assert topology.PARENT_V3_RUNTIME_FILE_SHA256 == _sha256(PARENT_RUNTIME_PATH)
    parent_runtime = json.loads(PARENT_RUNTIME_PATH.read_text(encoding="utf-8"))
    assert parent_runtime["manifest_hash"] == topology.PARENT_V3_RUNTIME_HASH
    assert topology.PARENT_V3_LAUNCH_RECORD_FILE_SHA256 == _sha256(
        PARENT_LAUNCH_RECORD_PATH
    )
    assert topology.PARENT_IDENTITY_V4_RUNTIME_FILE_SHA256 == _sha256(
        PARENT_IDENTITY_RUNTIME_PATH
    )
    parent_identity = json.loads(PARENT_IDENTITY_RUNTIME_PATH.read_text(encoding="utf-8"))
    assert parent_identity["manifest_hash"] == topology.PARENT_IDENTITY_V4_RUNTIME_HASH
...
def test_http_3xx_records_only_safe_metadata_without_follow_or_body_read() -> None:
    opener = _RedirectOpener(
        topology.SEED_URLS[0],
        "https://www.mexc.com/support/articles/?private=discard#fragment",
    )
    ...
    assert opener.body.read_calls == 0
    assert opener.body.closed is True
    assert fetched.body == b""
    result = topology.analyze_topology_responses(
        [fetched, *_synthetic_responses()[1:]]
    )
    record = result["records"][0]
    assert record["http_status_code"] == 302
    assert record["redirect_location"] == "https://www.mexc.com/support/articles/"
    assert record["response_body_read"] is False
    assert record["response_sha256"] is None
    assert record["response_bytes"] == 0
    assert record["content_type"] is None
    assert record["same_host_candidate_index_urls"] == []
    assert record["disposition"] == "REDIRECT_METADATA_RECORDED_NOT_FOLLOWED"
    encoded = json.dumps(record, sort_keys=True)
    assert "private" not in encoded
    assert "fragment" not in encoded


def test_http_3xx_rejects_cross_host_location_without_persisting_it() -> None:
    opener = _RedirectOpener(topology.SEED_URLS[0], "https://evil.example/secret")
    fetched = topology.fetch_official_topology_response(
        topology.SEED_URLS[0],
        capability=_approved_in_memory_capability(),
        deadline_monotonic=monotonic() + 30,
    ...
    record = result["records"][0]
    assert record["redirect_location"] is None
    assert (
        record["disposition"]
        == "REDIRECT_METADATA_REJECTED_UNSAFE_LOCATION_NOT_FOLLOWED"
    )
    assert "evil.example" not in json.dumps(record, sort_keys=True)
...
def test_manifest_is_code_test_and_launcher_bound() -> None:
    ...
    expected = topology.build_runtime_manifest(
        repo_root=ROOT,
        parent_runtime_manifest_path=PARENT_RUNTIME_PATH,
        parent_launch_record_path=PARENT_LAUNCH_RECORD_PATH,
        parent_identity_runtime_manifest_path=PARENT_IDENTITY_RUNTIME_PATH,
...
def test_preflight_reads_no_execution_manifest_and_has_no_side_effects() -> None:
    ...
    assert result["status"] == "BLOCKED_AWAIT_EXACT_V4_TOPOLOGY_EXECUTION_APPROVAL"
...
def test_launcher_is_fail_closed_and_prepares_output_only_after_writer_claim() -> None:
    ...
    assert "slow_liquidity_official_currentness_topology_v4" in launcher
    assert "Invoke-OfflinePreflight" in launcher
    assert "& $activeRunChecker -Json 2>&1" in launcher
    assert "& $activeRunChecker 2>&1" not in launcher
    assert "BLOCKED_AWAIT_EXACT_V4_TOPOLOGY_EXECUTION_APPROVAL" in launcher
    assert "RUN_SLOW_LIQUIDITY_OFFICIAL_CURRENTNESS_TOPOLOGY_DISCOVERY_V4" in launcher
    ...
    missing_manifest_index = launcher.index("exact_v4_execution_manifest_missing")
...
def test_launcher_preflight_and_direct_call_create_nothing() -> None:
    ...
    assert payload["status"] == "BLOCKED_AWAIT_EXACT_V4_TOPOLOGY_EXECUTION_APPROVAL"
...
def test_runtime_cli_has_preflight_but_no_execute_command() -> None:
    completed = subprocess.run(
        [
            sys.executable,
            "-m",
            "trading_mvp.src.slow_liquidity_official_currentness_topology_v4",
            "execute",
        ],
        cwd=ROOT,
        capture_output=True,
