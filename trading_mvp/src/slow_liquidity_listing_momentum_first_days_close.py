...
from slow_liquidity_listing_momentum_official_date import (
    DATE_PLAN_PATH as PARENT_DATE_PLAN_PATH,
    PLAN_ID as PARENT_DATE_PLAN_ID,
)
...
SCHEMA = "trading_mvp_slow_liquidity_listing_momentum_first_days_close_planonly_v1"
PLAN_ID = "slow_liquidity_listing_momentum_first_days_close_20260816"
HASH_METHOD = "sha256_canonical_json_excluding_plan_hash"
REPO_ROOT = Path(__file__).resolve().parents[2]
CLOSE_PLAN_PATH = (
    REPO_ROOT
    / "docs/plans"
    / "slow-liquidity-listing-momentum-first-days-close-planonly-20260816.json"
)
...
PARENT_DATE_PLAN_HASH = (
    "02883ef427104f25f36be1bfc349b6f41671ba5df0f96162bfa44ee4fb000337"
)
PARENT_DATE_PLAN_FILE_SHA256 = (
    "b169583600aba0aff3f10723494b0ea82d7e03f04dad822670d5cde9f2ddbc57"
)
PARENT_DATE_PLAN_STATUS = (
...
EXPECTED_APPROVAL_TEXT = (
    "CLOSE_LISTING_MOMENTUM_FIRST_DAYS_AS_INCOMPLETE — принимаю PlanOnly "
    "slow_liquidity_listing_momentum_first_days_close_20260816 по "
    "plan_hash=<PLAN_HASH> и plan_file_sha256=<PLAN_FILE_SHA256>: закрыть "
    "first-days Listing Momentum как incomplete, documented official date "
    "method нет, calendar listed_at не official announcement, first-days=0, "
    "не invent URL, не OHLCV, не replay v6, не reopen closed 9, не reopen "
    "listing-first, не reuse spot v2 consumer, не v7. Без evaluator, OOS, "
    "returns/PnL, grid/retune, paper/live, private API, реальных денег, "
    "плеча или маржи."
)


class ListingMomentumFirstDaysCloseError(ValueError):
    pass


def _require(value: bool, message: str) -> None:
    if not value:
        raise ListingMomentumFirstDaysCloseError(message)
...
def build_listing_momentum_first_days_close_plan(generated_at_utc: str) -> dict[str, Any]:
    ...
    if PARENT_DATE_PLAN_PATH.is_file():
        date_plan = json.loads(PARENT_DATE_PLAN_PATH.read_text(encoding="utf-8"))
        _require(
            _sha256_file(PARENT_DATE_PLAN_PATH) == PARENT_DATE_PLAN_FILE_SHA256,
            "parent official-date plan file hash mismatch",
        )
        _require(
            date_plan.get("plan_hash") == PARENT_DATE_PLAN_HASH,
            "parent official-date plan hash mismatch",
        )
        _require(
            date_plan.get("status") == PARENT_DATE_PLAN_STATUS,
            "parent official-date status mismatch",
        )
        _require(
            date_plan.get("documented_unsigned_announcement_json_endpoint") is False,
            "parent claimed announcement json",
        )
        _require(
            date_plan.get("close_listing_momentum_authorized") is False,
            "parent already authorized close",
        )
        _require(
            date_plan.get("network_authorized") is False,
            "parent official-date opened network",
        )
    plan: dict[str, Any] = {
        "schema": SCHEMA,
        "plan_id": PLAN_ID,
        ...
        "prepared_checkpoint": "CLOSE_LISTING_MOMENTUM_FIRST_DAYS_AS_INCOMPLETE",
        ...
        "documented_official_date_method_unavailable": True,
        "user_supplied_grounded_official_date_method": False,
        "market": "SPOT_USDT",
        ...
        "evidence_class": "LISTING_MOMENTUM_FIRST_DAYS_CLOSE_PACKET",
        "identity_before_ohlcv_collect": True,
        "two_venue_official_identity_complete": False,
        "listing_momentum_first_days_complete": False,
        "goal": (
            "Prepare close of first-days Listing Momentum as incomplete: "
            "no grounded official announcement/listing-date method was "
            "supplied, and the in-repo public trading-start fields are not "
            "official announcements. This is not OHLCV and not an identity "
            "verdict."
        ),
        ...
        "parent_official_date_method": {
            "plan_id": PARENT_DATE_PLAN_ID,
            "plan_path": str(PARENT_DATE_PLAN_PATH),
            "plan_hash": PARENT_DATE_PLAN_HASH,
            "parent_plan_file_sha256": PARENT_DATE_PLAN_FILE_SHA256,
            "status": PARENT_DATE_PLAN_STATUS,
...
def validate_listing_momentum_first_days_close_plan(plan: Mapping[str, Any]) -> None:
    _require(plan.get("schema") == SCHEMA, "first-days close schema mismatch")
    _require(plan.get("plan_id") == PLAN_ID, "first-days close plan id mismatch")
    ...
    _require(
        plan.get("documented_official_date_method_unavailable") is True,
        "official date method claimed available",
    )
    _require(
        plan.get("user_supplied_grounded_official_date_method") is False,
        "user method claimed supplied",
    )
    _require(
        plan.get("listing_momentum_first_days_complete") is False,
        "first-days claimed complete",
    )
    ...
    date_parent = plan.get("parent_official_date_method") or {}
    _require(date_parent.get("plan_hash") == PARENT_DATE_PLAN_HASH, "parent date hash")
    _require(
        date_parent.get("parent_plan_file_sha256") == PARENT_DATE_PLAN_FILE_SHA256,
        "parent date file hash",
    )
    _require(
        date_parent.get("documented_unsigned_announcement_json_endpoint") is False,
        "date parent claimed json",
    )
...
def write_listing_momentum_first_days_close_plan(generated_at_utc: str) -> Path:
    plan = build_listing_momentum_first_days_close_plan(generated_at_utc)
...
def main(argv: list[str] | None = None) -> int:
    ...
    path = write_listing_momentum_first_days_close_plan(generated)
