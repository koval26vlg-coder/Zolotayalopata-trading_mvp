...
RUN_ID = "slow_liquidity_official_currentness_topology_discovery_20260814_v2"
RUNTIME_MANIFEST_SCHEMA = (
    "trading_mvp_slow_liquidity_official_currentness_topology_runtime_manifest_v2"
)
RUNTIME_MANIFEST_STATUS = "FROZEN_OFFLINE_V2_AWAIT_EXACT_TOPOLOGY_EXECUTION_APPROVAL"
EXECUTION_MANIFEST_SCHEMA = (
    "trading_mvp_slow_liquidity_official_currentness_topology_execution_manifest_v2"
)
EXECUTION_APPROVED_STATUS = "FROZEN_V2_WITH_EXACT_TOPOLOGY_EXECUTION_APPROVAL"
EXECUTION_RECEIPT_SCHEMA = (
    "trading_mvp_slow_liquidity_official_currentness_topology_"
    "execution_approval_receipt_v2"
)
SANITIZED_OUTPUT_SCHEMA = (
    "trading_mvp_slow_liquidity_official_currentness_topology_sanitized_v2"
)
SANITIZED_OUTPUT_MANIFEST_SCHEMA = (
    "trading_mvp_slow_liquidity_official_currentness_topology_output_manifest_v2"
)

PROPOSAL_FILE_SHA256 = (
    "c79faa302577051af7d0328dcb11f473f0b61419076f973e4f4778b1e1de128c"
)
PROPOSAL_HASH = "05e27bbcf824b7413aaae8e08f059e83343b0c1edfc90ee61308611132faba94"
POSTMORTEM_FILE_SHA256 = (
    "3c785e6af4ca9eba3557444eddf6088a9b04e2f979698fdd7af61e9fd29aff75"
)
POSTMORTEM_AUDIT_HASH = (
    "09cf46a5fad0c8d8de3a65e5c6ffdfb35015fc5682906d55c32cc9f793d3a7db"
)
PARENT_TERMINAL_RUN_ID = (
    "slow_liquidity_official_currentness_topology_discovery_20260813_v1"
)
...
POSTMORTEM_PATH = (
    ...
    "topology-postmortem-20260814-v1.json"
)
RUNTIME_MODULE_PATH = Path(__file__).resolve()
SYNTHETIC_TESTS_PATH = (
    REPO_ROOT
    / "trading_mvp/tests/test_slow_liquidity_official_currentness_topology_v2.py"
)
...
SANITIZED_FAILURE_SCHEMA = (
    "trading_mvp_slow_liquidity_official_currentness_topology_failure_v2"
)
...
def _validate_exact_proposal(
    ...
    *,
    ...
    _require(
        proposal.get("schema")
        == "trading_mvp_slow_liquidity_official_currentness_topology_v2_refreeze_proposal_v1",
    ...
    _require(
        ...
        == "slow_liquidity_official_currentness_topology_v2_refreeze_20260814_v2",
    ...
    parent = proposal.get("parent_terminal_run")
    _require(isinstance(parent, dict), "terminal parent binding is missing")
    _require(
        set(parent) == {"run_id", "status", "retry_authorized", "postmortem"},
        "terminal parent field set changed",
    )
    _require(parent.get("run_id") == PARENT_TERMINAL_RUN_ID, "parent run mismatch")
    _require(parent.get("status") == "STOPPED_INCOMPLETE", "parent status mismatch")
    _require(parent.get("retry_authorized") is False, "parent retry was enabled")
    postmortem_ref = parent.get("postmortem")
    _require(isinstance(postmortem_ref, dict), "postmortem binding is missing")
    _require(
        postmortem_ref
        == {
            "path": str(POSTMORTEM_PATH),
            "file_sha256": POSTMORTEM_FILE_SHA256,
            "size_bytes": 5484,
            "audit_hash": POSTMORTEM_AUDIT_HASH,
        },
        "postmortem binding mismatch",
    )
    _, postmortem = _read_exact_json(
        POSTMORTEM_PATH,
        label="terminal parent postmortem",
        expected_file_sha256=POSTMORTEM_FILE_SHA256,
    )
    _require(POSTMORTEM_PATH.stat().st_size == 5484, "postmortem size mismatch")
    _require(
        postmortem.get("schema")
        == "trading_mvp_slow_liquidity_official_currentness_topology_postmortem_v1",
        "postmortem schema mismatch",
    )
    _require(
        postmortem.get("status") == "TERMINAL_REJECT_STOPPED_INCOMPLETE_NO_RETRY",
        "postmortem terminal status mismatch",
    )
    _require(
        postmortem.get("run_id") == PARENT_TERMINAL_RUN_ID,
        "postmortem run mismatch",
    )
    _require(
        postmortem.get("audit_hash") == POSTMORTEM_AUDIT_HASH,
        "postmortem hash mismatch",
    )
    _require(
        canonical_hash_without(postmortem, "audit_hash") == POSTMORTEM_AUDIT_HASH,
        "postmortem canonical hash mismatch",
    )

    candidate = proposal.get("proposed_successor")
    _require(isinstance(candidate, dict), "v2 successor is missing")
    _require(
        set(candidate)
        == {
            "run_id",
        ...
        "v2 successor field set changed",
    )
    ...
    _require(isinstance(supersedes, dict), "superseded proposal binding is missing")
    superseded_path = _local_repo_path(
        str(supersedes.get("path", "")),
        repo_root=root,
        label="superseded topology proposal",
    )
    _, superseded = _read_exact_json(
        superseded_path,
        label="superseded topology proposal",
        expected_file_sha256="a5bab6780168965c1006d8e10f8effb86214660b258fb00131a8f28926ff2399",
    )
    _require(
        superseded.get("proposal_hash")
        == "95be7057f3536653778dec92bb3a2114c86c160a50dd3aece4bef83a7927deb4",
        "superseded proposal hash mismatch",
    )
...
def _preflight_contract() -> dict[str, Any]:
    return {
        "status": "BLOCKED_AWAIT_EXACT_V2_TOPOLOGY_EXECUTION_APPROVAL",
...
def build_runtime_manifest(
    *,
    ...
    manifest: dict[str, Any] = {
        ...
        "parent_terminal": {
            "run_id": PARENT_TERMINAL_RUN_ID,
            "status": "STOPPED_INCOMPLETE",
            "retry_authorized": False,
            "postmortem_path": str(POSTMORTEM_PATH),
            "postmortem_file_sha256": POSTMORTEM_FILE_SHA256,
            "postmortem_audit_hash": POSTMORTEM_AUDIT_HASH,
        },
...
def preflight_only(
    *,
    ...
    return {
        "status": "BLOCKED_AWAIT_EXACT_V2_TOPOLOGY_EXECUTION_APPROVAL",
        "reason_code": "EXACT_V2_NETWORK_EXECUTION_APPROVAL_REQUIRED",
        "run_id": RUN_ID,
