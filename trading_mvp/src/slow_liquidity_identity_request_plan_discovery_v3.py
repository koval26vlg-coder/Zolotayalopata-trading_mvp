...
RUN_ID = "slow_liquidity_identity_request_plan_discovery_20260815_v3"
RUNTIME_MANIFEST_SCHEMA = (
    "trading_mvp_slow_liquidity_identity_request_plan_discovery_runtime_manifest_v3"
)
RUNTIME_MANIFEST_STATUS = (
    "FROZEN_OFFLINE_V3_AWAIT_EXACT_REQUEST_PLAN_DISCOVERY_EXECUTION_APPROVAL"
)
EXECUTION_MANIFEST_SCHEMA = (
    "trading_mvp_slow_liquidity_identity_request_plan_discovery_execution_manifest_v3"
)
EXECUTION_APPROVED_STATUS = (
    "FROZEN_V3_WITH_EXACT_REQUEST_PLAN_DISCOVERY_EXECUTION_APPROVAL"
)
...
APPROVAL_RECEIPT_PATH = (
    REPO_ROOT
    / "docs/agent-log/approvals/2026-08-15-slow-liquidity-identity-request-"
    "plan-discovery-v3-execution-approval.json"
)
LAUNCH_RECORD_PATH = (
    REPO_ROOT
...
OUTPUT_PATH = Path(
    ...
    r"request-plan\slow_liquidity_identity_request_plan_discovery_20260815_v3"
)
...
class RequestPlanDiscoveryV3Error(ValueError):
    pass


@dataclass(frozen=True, slots=True)
class RequestPlanDiscoveryV3Result:
    status: str
...
@dataclass(frozen=True, slots=True)
class RequestPlanDiscoveryV3ExecutionCapability:
    run_id: str
...
def canonical_json_bytes(value: Any) -> bytes:
    try:
        ...
        raise RequestPlanDiscoveryV3Error("payload is not canonical JSON") from exc
...
def _require(condition: bool, message: str) -> None:
    if not condition:
        raise RequestPlanDiscoveryV3Error(message)
...
def _validate_timestamp(value: Any, label: str) -> str:
    ...
    try:
        ...
        raise RequestPlanDiscoveryV3Error(f"{label} is invalid") from exc
...
def _reject_duplicate_json_keys(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
    ...
    for key, value in pairs:
        if key in result:
            raise RequestPlanDiscoveryV3Error(f"duplicate JSON key: {key}")
        result[key] = value
    return result
...
def _read_json(path: str | Path, label: str) -> tuple[bytes, dict[str, Any]]:
    ...
    try:
        ...
        raise RequestPlanDiscoveryV3Error(f"{label} is unavailable") from exc
    ...
    try:
        payload = json.loads(
            ...
            parse_constant=lambda value: (_ for _ in ()).throw(
                RequestPlanDiscoveryV3Error(f"invalid JSON constant: {value}")
            ),
        )
    except (UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise RequestPlanDiscoveryV3Error(f"{label} is invalid JSON") from exc
...
def _sha256_file(path: str | Path) -> str:
    try:
        ...
        raise RequestPlanDiscoveryV3Error(f"required file is unavailable: {path}") from exc
...
def _validate_lineage() -> dict[str, Any]:
    ...
    try:
        ...
        raise RequestPlanDiscoveryV3Error("parent discovery PlanOnly is invalid") from exc
    ...
    try:
        ...
        raise RequestPlanDiscoveryV3Error("topology v4 runtime is invalid") from exc
    _require(
    ...
    try:
        ...
        raise RequestPlanDiscoveryV3Error("topology v4 execution is invalid") from exc
...
def write_runtime_manifest(
    ...
    try:
        ...
        raise RequestPlanDiscoveryV3Error("runtime manifest already exists") from exc
    return target
...
def _load_exact_parent_plan(plan: Mapping[str, Any]) -> dict[str, Any]:
    try:
        ...
        raise RequestPlanDiscoveryV3Error("exact parent plan is unavailable") from exc
    if _sha256_bytes(raw) != PARENT_DISCOVERY_PLAN_FILE_SHA256:
        raise RequestPlanDiscoveryV3Error("exact parent plan file hash mismatch")
    if frozen.get("plan_hash") != PARENT_DISCOVERY_PLAN_HASH:
        raise RequestPlanDiscoveryV3Error("exact parent plan canonical hash mismatch")
    if dict(plan) != frozen:
        raise RequestPlanDiscoveryV3Error("exact parent plan binding mismatch")
    try:
        discovery_v2.validate_discovery_plan(frozen)
    except discovery_v2.RequestPlanDiscoveryError as exc:
        raise RequestPlanDiscoveryV3Error("exact parent plan is invalid") from exc
    return frozen


def _validated_response(response: FetchedResponse, expected_url: str) -> bytes:
    if type(response) is not FetchedResponse:
        raise RequestPlanDiscoveryV3Error("fetch response type changed")
    if response.requested_url != expected_url:
        raise RequestPlanDiscoveryV3Error("requested URL mismatch")
    if response.final_url != expected_url or 300 <= int(response.status) < 400:
        raise RequestPlanDiscoveryV3Error("HTTP redirect is forbidden")
    if response.status != 200:
        raise RequestPlanDiscoveryV3Error("HTTP response status is not 200")
    if type(response.body) is not bytes:
        raise RequestPlanDiscoveryV3Error("HTTP response body must be bytes")
    if not 0 < len(response.body) <= MAX_RESPONSE_BYTES:
        raise RequestPlanDiscoveryV3Error("HTTP response body exceeds cap")
...
def discover_request_plan(
    ...
    *,
    fetch: Callable[[str], FetchedResponse],
    monotonic: Callable[[], float] = time.monotonic,
) -> RequestPlanDiscoveryV3Result:
    frozen = _load_exact_parent_plan(plan)
    started = monotonic()
    responses: dict[str, FetchedResponse] = {}
    attempts: dict[str, int] = {}

    def fetch_once(url: str) -> FetchedResponse:
        if monotonic() - started > MAX_RUNTIME_SEC:
            raise RequestPlanDiscoveryV3Error("request-plan discovery deadline exceeded")
        if len(responses) >= MAX_TOTAL_HTTP_REQUESTS:
            raise RequestPlanDiscoveryV3Error("request-plan discovery request cap exceeded")
        attempts[url] = attempts.get(url, 0) + 1
        if attempts[url] > MAX_ATTEMPTS_PER_URL:
            raise RequestPlanDiscoveryV3Error("attempt cap per URL exceeded")
        response = fetch(url)
        _validated_response(response, url)
        responses[url] = response
        return response

    for venue in VENUES:
        fetch_once(OFFICIAL_METADATA_ENDPOINTS[venue])
    for item in frozen["seed_items"]:
        search_url = str(item["search_url"])
        search_response = fetch_once(search_url)
        try:
            ...
            raise RequestPlanDiscoveryV3Error(
    ...
    return RequestPlanDiscoveryV3Result(
        status=(
    ...
    *,
    ...
    request = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "trading-mvp-request-plan-discovery-v3/1.0",
        },
    )
    ...
    try:
        with opener.open(request, timeout=timeout_sec) as response:
            ...
            if declared is not None:
                _require(int(declared) <= MAX_RESPONSE_BYTES, "HTTP response body exceeds cap")
            body = response.read(MAX_RESPONSE_BYTES + 1)
    except urllib.error.HTTPError as exc:
        if 300 <= int(exc.code) < 400:
            raise RequestPlanDiscoveryV3Error("HTTP redirect is forbidden") from exc
        raise RequestPlanDiscoveryV3Error("HTTP response status is not 200") from exc
    except (urllib.error.URLError, TimeoutError, OSError) as exc:
        raise RequestPlanDiscoveryV3Error("public read-only HTTP request failed") from exc
    _require(status == 200, "HTTP response status is not 200")
    _require(final_url == url, "HTTP redirect is forbidden")
    _require(0 < len(body) <= MAX_RESPONSE_BYTES, "HTTP response body exceeds cap")
...
def validate_execution_manifest(
    ...
    *,
...
) -> RequestPlanDiscoveryV3ExecutionCapability:
    ...
    return RequestPlanDiscoveryV3ExecutionCapability(
        run_id=RUN_ID,
...
def collect_request_plan(
    *,
    capability: RequestPlanDiscoveryV3ExecutionCapability,
    fetch: Callable[[str], FetchedResponse] | None = None,
    monotonic: Callable[[], float] = time.monotonic,
) -> RequestPlanDiscoveryV3Result:
    _require(type(capability) is RequestPlanDiscoveryV3ExecutionCapability, "execution capability is required")
    _require(capability.run_id == RUN_ID, "execution capability run mismatch")
    started = monotonic()

    def bounded_fetch(url: str) -> FetchedResponse:
        remaining = MAX_RUNTIME_SEC - (monotonic() - started)
        _require(remaining > 0, "request-plan discovery deadline exceeded")
        if fetch is not None:
            return fetch(url)
        return fetch_public_response(url, timeout_sec=min(30.0, remaining))
...
def _write_exclusive(path: Path, payload: bytes) -> None:
    try:
        ...
        raise RequestPlanDiscoveryV3Error(f"immutable output already exists: {path.name}") from exc


def write_request_plan_bundle(
    output_path: str | Path,
    result: RequestPlanDiscoveryV3Result,
    *,
    capability: RequestPlanDiscoveryV3ExecutionCapability,
) -> dict[str, Any]:
    _require(type(capability) is RequestPlanDiscoveryV3ExecutionCapability, "execution capability is required")
    ...
    for item in request_plan:
        try:
            ...
            raise RequestPlanDiscoveryV3Error("request plan is not consumer-compatible") from exc
    request_payload = canonical_json_bytes(request_plan)
    manifest: dict[str, Any] = {
        "schema": "trading_mvp_slow_liquidity_identity_request_plan_discovery_output_manifest_v3",
    ...
    try:
        ...
        raise RequestPlanDiscoveryV3Error("immutable request-plan output already exists") from exc
    try:
...
def sanitized_failure_envelope(
    ...
    *,
    ...
    elif isinstance(error, RequestPlanDiscoveryV3Error):
        reason = "REQUEST_PLAN_DISCOVERY_RUNTIME_CONTRACT_REJECTED"
    else:
        reason = "REQUEST_PLAN_DISCOVERY_INTERNAL_RUNTIME_FAILURE"
    return {
        "schema": "trading_mvp_slow_liquidity_identity_request_plan_discovery_failure_v3",
