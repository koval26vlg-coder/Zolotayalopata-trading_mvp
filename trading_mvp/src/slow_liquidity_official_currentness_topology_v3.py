...
RUN_ID = "slow_liquidity_official_currentness_topology_discovery_20260814_v3"
RUNTIME_MANIFEST_SCHEMA = (
    "trading_mvp_slow_liquidity_official_currentness_topology_runtime_manifest_v3"
)
RUNTIME_MANIFEST_STATUS = "FROZEN_OFFLINE_V3_AWAIT_EXACT_TOPOLOGY_EXECUTION_APPROVAL"
EXECUTION_MANIFEST_SCHEMA = (
    "trading_mvp_slow_liquidity_official_currentness_topology_execution_manifest_v3"
)
EXECUTION_APPROVED_STATUS = "FROZEN_V3_WITH_EXACT_TOPOLOGY_EXECUTION_APPROVAL"
EXECUTION_RECEIPT_SCHEMA = (
    "trading_mvp_slow_liquidity_official_currentness_topology_"
    "execution_approval_receipt_v3"
)
SANITIZED_OUTPUT_SCHEMA = (
    "trading_mvp_slow_liquidity_official_currentness_topology_sanitized_v3"
)
SANITIZED_OUTPUT_MANIFEST_SCHEMA = (
    "trading_mvp_slow_liquidity_official_currentness_topology_output_manifest_v3"
)

PROPOSAL_FILE_SHA256 = (
    "e34526bcd672e86d4ae6c4d722aea63ffd9ee86ef51d1884589d184868e9e657"
)
PROPOSAL_HASH = "8d4e58344ae91c731de29c34786e00c3a682445f635b617e850d24b3b5770364"
INTEGRITY_AUDIT_FILE_SHA256 = (
    "c919adb83b4e0674aa321fa03d52453930c0c0f81c5babd04ce9ac9bc6f7bbd0"
)
INTEGRITY_AUDIT_HASH = (
    "527f4f120234eb36ffd4ac6b1554f0fafcc9dcf9f976143cd7e466aba97fe51a"
)
INTEGRITY_FINDING_ID = "V2_ACTIVE_RUN_GATE_JSON_MODE_MISSING"
SUPERSEDED_V2_RUN_ID = (
    "slow_liquidity_official_currentness_topology_discovery_20260814_v2"
)
SUPERSEDED_V2_PROPOSAL_FILE_SHA256 = (
    "c79faa302577051af7d0328dcb11f473f0b61419076f973e4f4778b1e1de128c"
)
SUPERSEDED_V2_PROPOSAL_HASH = (
    "05e27bbcf824b7413aaae8e08f059e83343b0c1edfc90ee61308611132faba94"
)
SUPERSEDED_V2_RUNTIME_FILE_SHA256 = (
    "05e13fbb0524f0ea3dc7f4d3f6003011037c02e7d922b74454188a9a22db4e94"
)
SUPERSEDED_V2_RUNTIME_HASH = (
    "921106cf8e6c9df44759480ad8f7a895f1f98d574d8c0c26b1dd10a87ae8c499"
)
SUPERSEDED_V2_LAUNCHER_FILE_SHA256 = (
    "4d5ae74e0d3e8169d220b4e4d38ac0c339b8f867b0d368475a552ca64a3937a3"
)
...
PROPOSAL_PATH = (
    REPO_ROOT / "docs/plans/drafts/slow-liquidity-official-currentness-topology-"
    "v3-refreeze-proposal-20260814-v1.json"
)
INTEGRITY_AUDIT_PATH = (
    REPO_ROOT / "docs/agent-log/readiness/slow-liquidity-official-currentness-"
    "topology-v2-launcher-gate-json-integrity-audit-20260814-v1.json"
)
SUPERSEDED_V2_PROPOSAL_PATH = (
    REPO_ROOT / "docs/plans/drafts/slow-liquidity-official-currentness-topology-"
    "v2-refreeze-proposal-20260814-v2.json"
)
SUPERSEDED_V2_RUNTIME_MANIFEST_PATH = (
    REPO_ROOT / "docs/plans/slow-liquidity-official-currentness-topology-runtime-"
    "manifest-20260814-v2.json"
)
SUPERSEDED_V2_LAUNCHER_PATH = (
...
SYNTHETIC_TESTS_PATH = (
    REPO_ROOT
    / "trading_mvp/tests/test_slow_liquidity_official_currentness_topology_v3.py"
)
...
APPROVAL_RECEIPT_PATH = (
    ...
    "currentness-topology-execution-v3-approval.json"
)
VISIBLE_LAUNCHER_PATH = (
    REPO_ROOT / "tools/start_exact_approved_slow_liquidity_official_currentness_"
    "topology_v3_visible.ps1"
)
OUTPUT_PATH = Path(
...
SANITIZED_FAILURE_SCHEMA = (
    "trading_mvp_slow_liquidity_official_currentness_topology_failure_v3"
)
...
def _validate_exact_proposal(
    ...
    *,
    ...
    _require(
        proposal.get("schema")
        == "trading_mvp_slow_liquidity_official_currentness_topology_v3_refreeze_proposal_v1",
        "topology proposal schema mismatch",
    )
    _require(
        proposal.get("status") == "PLANONLY_AWAIT_EXACT_OFFLINE_REFREEZE_APPROVAL",
        "topology proposal status mismatch",
    )
    _require(
        proposal.get("proposal_id")
        == "slow_liquidity_official_currentness_topology_v3_refreeze_20260814_v1",
        "topology proposal id mismatch",
    )
    _validate_timestamp(proposal.get("generated_at_utc"), "topology proposal timestamp")
    ...
    audit_ref = proposal.get("defect_audit")
    _require(
        audit_ref
        == {
            "path": str(INTEGRITY_AUDIT_PATH),
            "file_sha256": INTEGRITY_AUDIT_FILE_SHA256,
            "audit_hash": INTEGRITY_AUDIT_HASH,
            "finding_id": INTEGRITY_FINDING_ID,
        },
        "integrity audit binding mismatch",
    )
    _, audit = _read_exact_json(
        INTEGRITY_AUDIT_PATH,
        label="topology v2 launcher integrity audit",
        expected_file_sha256=INTEGRITY_AUDIT_FILE_SHA256,
    )
    _require(INTEGRITY_AUDIT_PATH.stat().st_size == 4050, "integrity audit size mismatch")
    _require(
        audit.get("schema")
        == "trading_mvp_slow_liquidity_official_currentness_topology_v2_launcher_gate_json_integrity_audit_v1",
        "integrity audit schema mismatch",
    )
    _require(
        audit.get("status") == "CRITICAL_BLOCKED_BEFORE_NETWORK_V2_NOT_LAUNCHABLE",
        "integrity audit status mismatch",
    )
    _require(audit.get("audit_hash") == INTEGRITY_AUDIT_HASH, "audit hash mismatch")
    _require(
        canonical_hash_without(audit, "audit_hash") == INTEGRITY_AUDIT_HASH,
        "audit canonical hash mismatch",
    )
    finding = audit.get("finding")
    _require(isinstance(finding, dict), "integrity finding is missing")
    _require(
        finding.get("finding_id") == INTEGRITY_FINDING_ID
        and finding.get("actual_invocation") == "& $activeRunChecker 2>&1"
        and finding.get("required_invocation") == "& $activeRunChecker -Json 2>&1"
        and finding.get("json_parse_succeeds") is False
        and finding.get("network_accessed") is False,
        "integrity finding changed",
    )
    side_effects = audit.get("side_effect_audit")
    _require(isinstance(side_effects, dict), "integrity side-effect audit is missing")
    _require(
        all(value is False for value in side_effects.values()),
        "v2 integrity audit reports side effects",
    )

    superseded = proposal.get("superseded_v2")
    expected_superseded = {
        "run_id": SUPERSEDED_V2_RUN_ID,
        "status": "FROZEN_OFFLINE_NEVER_EXECUTED_NOT_LAUNCHABLE",
        "proposal_path": str(SUPERSEDED_V2_PROPOSAL_PATH),
        "proposal_file_sha256": SUPERSEDED_V2_PROPOSAL_FILE_SHA256,
        "proposal_hash": SUPERSEDED_V2_PROPOSAL_HASH,
        "runtime_manifest_path": str(SUPERSEDED_V2_RUNTIME_MANIFEST_PATH),
        "runtime_manifest_file_sha256": SUPERSEDED_V2_RUNTIME_FILE_SHA256,
        "runtime_manifest_hash": SUPERSEDED_V2_RUNTIME_HASH,
        "launcher_path": str(SUPERSEDED_V2_LAUNCHER_PATH),
        "launcher_file_sha256": SUPERSEDED_V2_LAUNCHER_FILE_SHA256,
        "retry_authorized": False,
        "execution_authorized": False,
    }
    _require(superseded == expected_superseded, "superseded v2 binding mismatch")
    _, v2_proposal = _read_exact_json(
        SUPERSEDED_V2_PROPOSAL_PATH,
        label="superseded topology v2 proposal",
        expected_file_sha256=SUPERSEDED_V2_PROPOSAL_FILE_SHA256,
    )
    _require(
        v2_proposal.get("proposal_hash") == SUPERSEDED_V2_PROPOSAL_HASH
        and canonical_hash_without(v2_proposal, "proposal_hash")
        == SUPERSEDED_V2_PROPOSAL_HASH,
        "superseded v2 proposal hash mismatch",
    )
    _, v2_runtime = _read_exact_json(
        SUPERSEDED_V2_RUNTIME_MANIFEST_PATH,
        label="superseded topology v2 runtime manifest",
        expected_file_sha256=SUPERSEDED_V2_RUNTIME_FILE_SHA256,
    )
    _require(
        v2_runtime.get("manifest_hash") == SUPERSEDED_V2_RUNTIME_HASH
        and canonical_hash_without(v2_runtime, "manifest_hash")
        == SUPERSEDED_V2_RUNTIME_HASH,
        "superseded v2 runtime hash mismatch",
    )
    _require(
        _sha256_file(SUPERSEDED_V2_LAUNCHER_PATH)
        == SUPERSEDED_V2_LAUNCHER_FILE_SHA256,
        "superseded v2 launcher hash mismatch",
    )

    candidate = proposal.get("proposed_successor")
    _require(isinstance(candidate, dict), "v3 successor is missing")
    _require(
        set(candidate)
        == {
            "run_id",
        ...
        "v3 successor field set changed",
    )
    _require(candidate.get("run_id") == RUN_ID, "topology run_id mismatch")
    _require(
        candidate.get("runtime_module_path") == str(RUNTIME_MODULE_PATH),
        "topology runtime module path mismatch",
    )
    _require(
        candidate.get("runtime_manifest_path") == str(RUNTIME_MANIFEST_PATH),
        "topology runtime manifest path mismatch",
    )
    _require(
        candidate.get("launcher_path") == str(VISIBLE_LAUNCHER_PATH),
        "topology launcher path mismatch",
    )
    _require(
        candidate.get("synthetic_tests_path") == str(SYNTHETIC_TESTS_PATH),
        "topology synthetic tests path mismatch",
    )
    _require(
        candidate.get("output_path") == str(OUTPUT_PATH),
        "topology output path mismatch",
    )
    _require(
    ...
    _require(
        ...
        "topology limits mismatch",
    )
    _require(
        proposal.get("exact_fix_contract")
        == {
            "active_run_gate_invocation": "& $activeRunChecker -Json 2>&1",
            "active_run_gate_json_mode_required": True,
            "active_run_gate_status_running_blocks": True,
            "malformed_or_non_json_gate_output_blocks": True,
            "no_writer_claim_before_all_preflight_checks": True,
            "no_visible_terminal_before_all_preflight_checks": True,
            "no_network_before_visible_worker_ownership_and_writer_claim": True,
            "v2_runtime_and_launcher_remain_immutable": True,
        },
        "exact gate fix contract mismatch",
    )
    _require(
        proposal.get("requested_offline_scope")
        == {
            "new_run_id_code_refreeze": True,
            "active_run_gate_json_contract_fix": True,
            "sanitized_failure_envelope": True,
            "synthetic_tests": True,
            "real_checker_contract_regression": True,
            "immutable_runtime_manifest": True,
            ...
            "writer_claim": False,
            "topology_output": False,
            "identity_evidence": False,
            "request_plan": False,
            "currentness_verdict": False,
            "visible_launcher_execution": False,
        },
        "offline scope mismatch",
    )
    _require(
        proposal.get("safety")
        == {
            ...
            "stopped_incomplete_retry": False,
        },
        "proposal safety contract mismatch",
    )
    _require(
        proposal.get("approval_effect")
        == "OFFLINE_REFREEZE_ONLY_NETWORK_REMAINS_BLOCKED",
        "proposal approval effect mismatch",
    )
    _require(
        proposal.get("separate_network_execution_approval_required") is True,
        "separate network approval requirement changed",
    )
    _require(
        proposal.get("proposal_hash_method")
        == "sha256_canonical_json_excluding_proposal_hash",
        "proposal hash method mismatch",
    )
...
def build_runtime_manifest(
    *,
    ...
    root = _validated_repo_root(repo_root)
    proposal, _ = _validate_exact_proposal(proposal_path, repo_root=root)
    ...
    _require(
        proposal["proposed_successor"]["run_id"] == RUN_ID,
...
def preflight_only(
    *,
    ...
    return {
        "status": "BLOCKED_AWAIT_EXACT_V3_TOPOLOGY_EXECUTION_APPROVAL",
        "reason_code": "EXACT_V3_NETWORK_EXECUTION_APPROVAL_REQUIRED",
        "run_id": RUN_ID,
