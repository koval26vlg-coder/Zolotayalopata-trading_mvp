...
RUN_ID = "slow_liquidity_official_currentness_topology_discovery_20260814_v4"
RUNTIME_MANIFEST_SCHEMA = (
    "trading_mvp_slow_liquidity_official_currentness_topology_runtime_manifest_v4"
)
RUNTIME_MANIFEST_STATUS = "FROZEN_OFFLINE_V4_AWAIT_EXACT_TOPOLOGY_EXECUTION_APPROVAL"
EXECUTION_MANIFEST_SCHEMA = (
    "trading_mvp_slow_liquidity_official_currentness_topology_execution_manifest_v4"
)
EXECUTION_APPROVED_STATUS = "FROZEN_V4_WITH_EXACT_TOPOLOGY_EXECUTION_APPROVAL"
EXECUTION_RECEIPT_SCHEMA = (
    "trading_mvp_slow_liquidity_official_currentness_topology_"
    "execution_approval_receipt_v4"
)
SANITIZED_OUTPUT_SCHEMA = (
    "trading_mvp_slow_liquidity_official_currentness_topology_sanitized_v4"
)
...
PARENT_V3_RUN_ID = (
    "slow_liquidity_official_currentness_topology_discovery_20260814_v3"
)
PARENT_V3_RUNTIME_FILE_SHA256 = (
    "3bb73c98c49b3440738c1d770932b4a504a74b56d8e25a9b9bd23532c5ff0c21"
)
PARENT_V3_RUNTIME_HASH = (
    "57bae89770da5656671d671d96793ced40a4f192b2d9d12009e6fedad035140b"
)
PARENT_V3_LAUNCH_RECORD_FILE_SHA256 = (
    "c43ffdc9c9d94217be448c067c3941fe33b52bc620742469711a305e6e3a7849"
)
...
ALLOWED_TERMINATION_MARKERS = frozenset(
    {
        ...
        "HTTP_REDIRECT_METADATA_RECORDED_BODY_NOT_READ",
    }
)
ALLOWED_DISPOSITIONS = frozenset(
    {
        ...
        "REDIRECT_METADATA_RECORDED_NOT_FOLLOWED",
        "REDIRECT_METADATA_REJECTED_UNSAFE_LOCATION_NOT_FOLLOWED",
    }
)

REPO_ROOT = Path(__file__).resolve().parents[2]
PARENT_V3_RUNTIME_MANIFEST_PATH = (
    REPO_ROOT / "docs/plans/slow-liquidity-official-currentness-topology-runtime-"
    "manifest-20260814-v3.json"
)
PARENT_V3_LAUNCH_RECORD_PATH = (
    REPO_ROOT / "docs/agent-log/run-gates/"
    f"{PARENT_V3_RUN_ID}.launch.json"
)
PARENT_IDENTITY_V4_RUNTIME_MANIFEST_PATH = (
    REPO_ROOT / "docs/plans/slow-liquidity-official-identity-runtime-manifest-"
    "20260813-v4.json"
)
RUNTIME_MODULE_PATH = Path(__file__).resolve()
SYNTHETIC_TESTS_PATH = (
    REPO_ROOT
    / "trading_mvp/tests/test_slow_liquidity_official_currentness_topology_v4.py"
)
...
EXECUTION_MANIFEST_PATH = (
    REPO_ROOT / "docs/plans/slow-liquidity-official-currentness-topology-execution-"
    "manifest-20260814-v4.json"
)
APPROVAL_RECEIPT_PATH = (
    REPO_ROOT / "docs/agent-log/approvals/2026-08-14-slow-liquidity-official-"
    "currentness-topology-execution-v4-approval.json"
)
VISIBLE_LAUNCHER_PATH = (
    REPO_ROOT / "tools/start_exact_approved_slow_liquidity_official_currentness_"
    "topology_v4_visible.ps1"
)
OUTPUT_PATH = Path(
...
SANITIZED_FAILURE_SCHEMA = (
    "trading_mvp_slow_liquidity_official_currentness_topology_failure_v4"
)
ALLOWED_FAILURE_REASON_CODES = frozenset(
    {
...
_FAILURE_REASON_MATCHERS = (
    ("redirect", "HTTP_REDIRECT_FORBIDDEN"),
    ("HTTP request failed", "OFFICIAL_TOPOLOGY_HTTP_REQUEST_FAILED"),
    ("deadline exceeded", "TOPOLOGY_RUNTIME_DEADLINE_EXCEEDED"),
    ("response cap exceeded", "RESPONSE_CAP_EXCEEDED"),
    ("content length", "CONTENT_LENGTH_EXCEEDS_RESPONSE_CAP"),
    ("compressed response", "COMPRESSED_RESPONSE_ENCODING_FORBIDDEN"),
    ("HTTP 200", "OFFICIAL_SOURCE_HTTP_STATUS_NOT_200"),
)
...
def sanitized_failure_envelope(
    ...
    *,
    ...
    """Return a fixed allowlisted failure record without exception or payload text."""
    reason_code = "TOPOLOGY_INTERNAL_RUNTIME_FAILURE"
    if isinstance(error, TopologyDiscoveryError):
        reason_code = "TOPOLOGY_RUNTIME_CONTRACT_REJECTED"
        message = str(error)
        for marker, candidate in _FAILURE_REASON_MATCHERS:
            if marker in message:
                reason_code = candidate
                break
    return {
        "schema": SANITIZED_FAILURE_SCHEMA,
...
class TopologyExecutionCapability:
    ...
    def __init__(
        ...
        *,
        ...
        object.__setattr__(self, "not_before_local", not_before_local)
        object.__setattr__(self, "latest_launch_local", latest_launch_local)
        object.__setattr__(self, "hard_deadline_local", hard_deadline_local)
...
def _expected_transport_requirements() -> dict[str, bool]:
    return {
        ...
        "redirect_response_metadata_allowed": True,
        "redirect_response_body_read_allowed": False,
        "redirect_location_sanitization_required": True,
...
def _expected_output_contract() -> dict[str, Any]:
    return {
        ...
        "redirect_metadata_is_not_identity_evidence": True,
        "redirect_body_hash_or_content_persistence_allowed": False,
    }


def _validate_refreeze_parents(
    *,
    repo_root: str | Path,
    parent_runtime_manifest_path: str | Path,
    parent_launch_record_path: str | Path,
    ...
    runtime_path = _local_repo_path(
        ...
        repo_root=root,
        label="parent topology v3 runtime manifest",
    )
    launch_path = _local_repo_path(
        parent_launch_record_path,
        repo_root=root,
        label="parent topology v3 launch record",
    )
    identity_path = _local_repo_path(
        parent_identity_runtime_manifest_path,
        repo_root=root,
        label="parent identity v4 runtime manifest",
    )
    _require(
        runtime_path == PARENT_V3_RUNTIME_MANIFEST_PATH,
        "parent topology runtime path mismatch",
    )
    _require(
        launch_path == PARENT_V3_LAUNCH_RECORD_PATH,
        "parent topology launch record path mismatch",
    )
    _require(
        identity_path == PARENT_IDENTITY_V4_RUNTIME_MANIFEST_PATH,
        "parent identity runtime path mismatch",
    )

    _, runtime = _read_exact_json(
        runtime_path,
        label="parent topology v3 runtime manifest",
        expected_file_sha256=PARENT_V3_RUNTIME_FILE_SHA256,
    )
    _require(
        runtime.get("schema")
        == "trading_mvp_slow_liquidity_official_currentness_topology_runtime_manifest_v3",
        "parent topology runtime schema mismatch",
    )
    _require(runtime.get("run_id") == PARENT_V3_RUN_ID, "parent topology run_id mismatch")
    _require(
        runtime.get("manifest_hash") == PARENT_V3_RUNTIME_HASH
        and canonical_hash_without(runtime, "manifest_hash") == PARENT_V3_RUNTIME_HASH,
        "parent topology runtime hash mismatch",
    )

    _, launch = _read_exact_json(
        launch_path,
        label="parent topology v3 launch record",
        expected_file_sha256=PARENT_V3_LAUNCH_RECORD_FILE_SHA256,
    )
    _require(
        launch.get("schema") == "trading_mvp_slow_liquidity_official_topology_launch_v3",
        "parent topology launch schema mismatch",
    )
    _require(
        launch.get("status") == "STOPPED_INCOMPLETE"
        and launch.get("run_id") == PARENT_V3_RUN_ID
        and launch.get("failure_reason_code") == "HTTP_REDIRECT_FORBIDDEN"
        and launch.get("topology_output_created") is False
        and launch.get("retry_authorized") is False,
        "parent topology launch disposition mismatch",
    )
    _require(
        launch.get("runtime_manifest_file_sha256") == PARENT_V3_RUNTIME_FILE_SHA256,
        "parent launch runtime binding mismatch",
    )

    _, identity = _read_exact_json(
        identity_path,
        label="parent identity v4 runtime manifest",
        expected_file_sha256=PARENT_IDENTITY_V4_RUNTIME_FILE_SHA256,
    )
    _require(
        identity.get("schema")
        == "trading_mvp_slow_liquidity_official_identity_runtime_manifest_v1",
        "parent identity runtime schema mismatch",
    )
    _require(
        identity.get("manifest_hash") == PARENT_IDENTITY_V4_RUNTIME_HASH
        and canonical_hash_without(identity, "manifest_hash")
        == PARENT_IDENTITY_V4_RUNTIME_HASH,
        "parent identity runtime hash mismatch",
    )
    identity_execution = identity.get("execution_authorization")
    _require(
        isinstance(identity_execution, dict)
        and identity_execution.get("approved") is False
        and identity_execution.get("actual_network_run_allowed") is False
        and identity_execution.get("identity_output_allowed") is False,
        "parent identity execution boundary mismatch",
    )

    return {
        "topology_v3_runtime": {
            "path": str(runtime_path),
            "file_sha256": PARENT_V3_RUNTIME_FILE_SHA256,
            "manifest_hash": PARENT_V3_RUNTIME_HASH,
            "run_id": PARENT_V3_RUN_ID,
        },
        "topology_v3_terminal_launch_record": {
            "path": str(launch_path),
            "file_sha256": PARENT_V3_LAUNCH_RECORD_FILE_SHA256,
            "status": "STOPPED_INCOMPLETE",
            "failure_reason_code": "HTTP_REDIRECT_FORBIDDEN",
            "retry_authorized": False,
            "topology_output_created": False,
        },
        "identity_runtime_v4": {
...
def _preflight_contract() -> dict[str, Any]:
    return {
        ...
        "parent_artifacts_must_be_read_and_validated": True,
    ...
    return {
        ...
        "allowlisted_fields": list(
            sanitized_failure_envelope(
                TopologyDiscoveryError("contract"),
                network_stage_entered=False,
            )
        ),
        "allowlisted_reason_codes": sorted(ALLOWED_FAILURE_REASON_CODES),
        "raw_exception_text_allowed": False,
        "traceback_allowed": False,
        "raw_payload_allowed": False,
        "unknown_exception_maps_to_internal_reason_code": True,
        "network_state_is_explicit": True,
        "retry_authorized": False,
    }
...
def build_runtime_manifest(
    *,
    ...
    launcher_path = _local_repo_path(
        visible_launcher_path,
        repo_root=root,
        label="topology visible launcher",
    )
    ...
    _require(launcher_path == VISIBLE_LAUNCHER_PATH, "visible launcher path mismatch")
    manifest: dict[str, Any] = {
        ...
        "research_only": True,
        "run_id": RUN_ID,
        "refreeze_parents": parent_bindings,
        "exact_change_contract": {
            "http_3xx_is_sanitized_topology_metadata": True,
            "http_3xx_status_code_persisted": True,
            "safe_location_persisted": True,
            "unsafe_location_persisted": False,
            "redirect_following_allowed": False,
            "redirect_body_read_allowed": False,
            "redirect_body_persistence_allowed": False,
        },
        ...
        "runtime": {
            ...
            "visible_launcher_sha256": _sha256_file(launcher_path),
            ...
            "sanitized_failure_envelope_implemented": True,
            "visible_launcher_implemented": True,
            "visible_launcher_execution_enabled": False,
        ...
        "sanitized_failure_contract": _failure_contract(),
...
def validate_runtime_manifest(
    ...
    *,
    ...
    expected = build_runtime_manifest(
        repo_root=repo_root,
        parent_runtime_manifest_path=PARENT_V3_RUNTIME_MANIFEST_PATH,
        parent_launch_record_path=PARENT_V3_LAUNCH_RECORD_PATH,
        parent_identity_runtime_manifest_path=PARENT_IDENTITY_V4_RUNTIME_MANIFEST_PATH,
        runtime_module_path=RUNTIME_MODULE_PATH,
        synthetic_tests_path=SYNTHETIC_TESTS_PATH,
        visible_launcher_path=VISIBLE_LAUNCHER_PATH,
...
def _sanitize_redirect_location(source_url: str, value: Any) -> str | None:
    if not isinstance(value, str) or not value or _CONTROL_CHARACTERS.search(value):
        return None
    ...
    sanitized = urllib.parse.urlunsplit(
        ("https", parsed.hostname or "", parsed.path or "/", "", "")
    )
    if len(sanitized.encode("utf-8")) > MAX_URL_BYTES:
        return None
    return sanitized
...
def _validate_response(response: FetchedResponse, expected_url: str) -> str | None:
    ...
    if 300 <= response.status < 400:
        _require(response.body == b"", "redirect response body must not be read")
        return None
    _require(response.status == 200, "official source must return HTTP 200 or sanitized 3xx")
...
def _record_for_response(response: FetchedResponse) -> dict[str, Any]:
    ...
    if 300 <= response.status < 400:
        location = _sanitize_redirect_location(
            ...
            _header_value(response.headers, "location"),
        )
        record = {
            ...
            "disposition": (
                ...
                else "REDIRECT_METADATA_REJECTED_UNSAFE_LOCATION_NOT_FOLLOWED"
            ),
        }
        _require(set(record) == ALLOWED_RECORD_FIELDS, "topology record field set changed")
        return record
    _require(content_type is not None, "HTTP 200 content type is missing")
...
def _validate_sanitized_result(result: Mapping[str, Any]) -> None:
    ...
    for index, record in enumerate(records):
        ...
        _require(
            isinstance(status, int) and (status == 200 or 300 <= status < 400),
        ...
        if 300 <= status < 400:
            location = record.get("redirect_location")
            _require(
                location is None
                or _sanitize_redirect_location(record["source_url"], location) == location,
                "redirect Location is not sanitized",
            )
            _require(record.get("response_body_read") is False, "redirect body was read")
            _require(record.get("response_sha256") is None, "redirect body hash was persisted")
            _require(record.get("response_bytes") == 0, "redirect body bytes were persisted")
            _require(record.get("content_type") is None, "redirect content type was persisted")
            _require(not candidates and not templates, "redirect produced topology candidates")
        else:
            _require(record.get("redirect_location") is None, "HTTP 200 has redirect metadata")
            _require(record.get("response_body_read") is True, "HTTP 200 body was not read")
            _require_hash(record.get("response_sha256"), "response hash")
            _require(isinstance(record.get("content_type"), str), "content type is invalid")
...
def _validated_launch_window(value: Any) -> dict[str, str]:
    _require(isinstance(value, dict), "execution launch window is missing")
    _require(
        set(value)
        == {"not_before_local", "latest_launch_local", "hard_deadline_local"},
        "execution launch window field set changed",
    )
    parsed: dict[str, datetime] = {}
    for field in ("not_before_local", "latest_launch_local", "hard_deadline_local"):
        raw = value.get(field)
        _require(isinstance(raw, str), f"execution {field} is invalid")
        try:
            parsed[field] = datetime.fromisoformat(raw.replace("Z", "+00:00"))
        except ValueError as exc:
            raise TopologyDiscoveryError(f"execution {field} is invalid") from exc
        _require(
            parsed[field].tzinfo is not None and parsed[field].utcoffset() is not None,
            f"execution {field} must be timezone-aware",
        )
    _require(
        parsed["not_before_local"] <= parsed["latest_launch_local"],
        "execution launch window is inverted",
    )
    _require(
        (parsed["hard_deadline_local"] - parsed["latest_launch_local"]).total_seconds()
        >= MAX_RUNTIME_SEC,
        "execution hard deadline cannot fit the runtime cap",
    )
    return {field: str(value[field]) for field in parsed}
...
def _validate_execution_receipt(
    ...
    *,
    ...
    launch_window = _validated_launch_window(receipt.get("launch_window"))
    _require(
...
def validate_execution_manifest(
    ...
    *,
    ...
    launch_window = _validated_launch_window(execution_manifest.get("launch_window"))
    _require(
    ...
    _require(validated_output_path == OUTPUT_PATH, "execution output path mismatch")
    _require(
    ...
    if approval_receipt_snapshot is None:
        ...
        _require(
            receipt_path == APPROVAL_RECEIPT_PATH, "approval receipt path mismatch"
        )
    ...
    _require(
        receipt.get("launch_window") == launch_window,
        "execution launch window binding mismatch",
    )
    return TopologyExecutionCapability(
        ...
        not_before_local=launch_window["not_before_local"],
        latest_launch_local=launch_window["latest_launch_local"],
        hard_deadline_local=launch_window["hard_deadline_local"],
        ...
        with transport.open(request, timeout=timeout) as response:
            ...
            if 300 <= status < 400:
                fetched = FetchedResponse(
                    ...
                    headers={
                        "location": _sanitize_redirect_location(
                            url, _header_value(response.headers, "location")
                        )
                        or ""
                    },
                    body=b"",
                ...
                return fetched
            _require(status == 200, "official source must return HTTP 200 or sanitized 3xx")
        ...
        if 300 <= exc.code < 400:
            try:
                fetched = FetchedResponse(
                    ...
                    headers={
                        "location": _sanitize_redirect_location(
                            url, _header_value(exc.headers, "location") if exc.headers else None
                        )
                        or ""
                    },
                    body=b"",
...
def preflight_only(
    *,
    ...
    parents = _validate_refreeze_parents(
        repo_root=root,
        parent_runtime_manifest_path=parent_runtime_manifest_path,
        parent_launch_record_path=parent_launch_record_path,
        parent_identity_runtime_manifest_path=parent_identity_runtime_manifest_path,
    ...
    _require(
        execution_path == EXECUTION_MANIFEST_PATH,
        "execution manifest path mismatch",
    )
    ...
    return {
        "status": "BLOCKED_AWAIT_EXACT_V4_TOPOLOGY_EXECUTION_APPROVAL",
        "reason_code": "EXACT_V4_NETWORK_EXECUTION_APPROVAL_REQUIRED",
        "run_id": RUN_ID,
        "refreeze_parents": parents,
...
def _parse_args() -> argparse.Namespace:
    ...
    freeze.add_argument("--parent-runtime-manifest", required=True)
    freeze.add_argument("--parent-launch-record", required=True)
    freeze.add_argument("--parent-identity-runtime-manifest", required=True)
    freeze.add_argument("--runtime-module", required=True)
    freeze.add_argument("--synthetic-tests", required=True)
    freeze.add_argument("--visible-launcher", required=True)
    ...
    preflight.add_argument("--parent-runtime-manifest", required=True)
    preflight.add_argument("--parent-launch-record", required=True)
    preflight.add_argument("--parent-identity-runtime-manifest", required=True)
...
def main() -> int:
    ...
    if args.command == "freeze-runtime":
        manifest = build_runtime_manifest(
            ...
            parent_runtime_manifest_path=args.parent_runtime_manifest,
            parent_launch_record_path=args.parent_launch_record,
            parent_identity_runtime_manifest_path=args.parent_identity_runtime_manifest,
            runtime_module_path=args.runtime_module,
            synthetic_tests_path=args.synthetic_tests,
            visible_launcher_path=args.visible_launcher,
